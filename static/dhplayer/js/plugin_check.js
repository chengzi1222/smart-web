...
function onPlayBegan(index)   //开始直播回调
{
    ...
    $('#error').html('<div class="warnImg">正在直播中...<a style="font-size:16px;color:#ff9007;display:block;" class="goSignin" href="javascript:;">点击返回预览信息填写页面</a></div>');
}
...
function StopOcx()
{
    ...
    $('#error').html('<div class="warnImg">提示：已经停止播放<a style="font-size:16px;color:#ff9007;display:block;" class="goSignin" href="javascript:;">点击返回预览信息填写页面</a></div>');
}
...
function CallOcx()    //1播放  0暂停
{
    ...
    var token = $('#item1').val();
    var deviceID = $('#item2').val();
    var channelID = parseInt($('#item3').val());
   //--初始化、装载样式文件
    if(1 == stopOrPlay)
    {
        toPlayBtn();//按钮状态
     ...
     // alert(token+","+deviceID+","+channelID+","+clarity);
        ...
        $('#error').html('<div class="warnImg loading">正在加载中...<a style="display:block;font-size:16px;color:#ff9007;" class="goSignin" href="javascript:;">点击返回预览信息填写页面</a></div>');
        if(iRes == -1)
        {
             $('#error').html('<div class="warnImg">提示：失败<a style="font-size:16px;color:#ff9007;display:block;" class="goSignin" href="javascript:;">点击返回预览信息填写页面</a></div>');
             return;
        }
...
function changeClarity()    //清晰度切换
{
    if(clarityBtnStatus){
        ...
        {
            $('#error').html('<div class="warnImg">提示：停止播放状态无法切换清晰度<a style="font-size:16px;color:#ff9007;display:block;" class="goSignin" href="javascript:;">点击返回预览信息填写页面</a></div>');
            return;
        }
        else
        {
