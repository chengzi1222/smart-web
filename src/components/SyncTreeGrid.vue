...
<template>
  <div>
    <el-button v-if="ade != 'detail'" type="primary" style="float: right; margin-bottom: 10px;"
               @click="addRow(data)">
      <i class="el-icon-plus el-icon--left"></i>添加字典项
    </el-button>
    <el-table :data="data" border style="width: 100%" :row-style="showTr">
      <el-table-column v-for="(column, index) in columns" :key="column.dataIndex" :label="column.text">
        <template slot-scope="scope">
          ...
          <span v-if="(scope.row._create && column.dataIndex != 'level') || (scope.row._edit && column.dataIndex != 'code' && column.dataIndex != 'level')">
            <input type="text" :value="scope.row[column.dataIndex]" @change="inputChange($event,column.dataIndex,scope.row)"></input>
            <span style="color: #ff4949" v-if="column.dataIndex == 'name' && nameIsNull">{{scope.row.nameIsNull}}</span>
            <span style="color: #ff4949" v-if="column.dataIndex == 'code' && codeIsNull">{{scope.row.codeIsNull}}</span>
          </span>
          <span v-else>
      ...
      <el-table-column label="操作" v-if="treeType === 'normal' && ade != 'detail'" width="260">
        <template slot-scope="scope">
...
<script>
  import Utils from 'utils/translate.js'
//  import ElButton from "../../node_modules/element-ui/packages/button/src/button";
  ...
  export default {
    ...
    mounted(){
//    setTimeout(() => {
//      this.dataSource = tanslate(this.datainit)
//    }, 1000);
      //debugger
    },
    watch: {
    ...
    methods: {
      ...
      btnClick(type, rowArr, index, arr) {
        ...
        if(type == "添加子字典项"){
          ...
          let path = rowArr["path"];  //获取当前path
          let obj = [{level:level+1,leaf:"YES",path:path}]; //初步定义要增加的子元素
...
//      }
        if(!rowArr.name) {
          ...
          rowArr.nameIsNull = '请输入名称';
          return false;
        }
        if(!rowArr.code) {
          this.codeIsNull = true;
          rowArr.codeIsNull = '请输入编码';
          return false;
        }
      ...
      addRow(data) {
        ...
        this.$set(data[0],'_edit',true);
        this.$set(data[0], '_create', true);
//      this.$set(data[0],'_expanded',true);
//      this.$set(data[0],'children',[]);
        this.$set(data[0],'leaf',"YES");
        this.$set(data[0],'level',1);
      }
    }
  }
  function tanslate(data) {
    ...
    if(data.length > 0) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (item.level !== 1) {
          if (item.leaf == "NO" && !item.children) item.children = [];
          console.log(data[level[item.level - 1]])
          console.log(level[item.level - 1])
