    ...
    <div class="titBox" :style="styleObject">
      ...
      <p>动态评级：
        <span v-if="dataLis.grade && dataLis.grade.length > 0">
          <span v-for="(item,index) in dataLis.grade" :key="index">{{item.level}}：{{item.number}}，</span>
        </span>
        <span v-else>暂无</span>
      </p>
      <p>巡查：共{{patrol}}次<span v-for="(item,index) in dataLis.result" :key="index">，{{item.resultStr}}：{{item.number}}</span></p>
    </div>
  </div>
</template>
  ...
  methods: {
    numChange(){
      ...
      for(let i=0;i<this.dataLis.result.length;i++){
        this.patrol += this.dataLis.result[i].number;
      }
    }
  },
  watch:{
