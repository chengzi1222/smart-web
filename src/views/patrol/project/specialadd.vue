...
<template>
    <div>
                ...
                <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="120px" class="demo-ruleForm">
                        ...
                        <div class="content">
                            ...
                            <div class="row" v-if="ruleForm.planName">
                                <el-form-item label="主体信息:" prop="">
                                    <select-mainbody :key="ruleForm.patrolPlanId" @slmainbody="slmainbody"
                                        :currentIndex='currentIndex' :apiFn='selectMainbodyApiFn' :params='params'></select-mainbody>
                                </el-form-item>
                            </div>
                            <!-- 选择的主体 -->
                            ...
                            <div class="row">
                                <el-form-item :label="areaCode.length === 6 || areaCode.length === 4 ? '检查人员:' : '监管人员:'" prop="patrolSoId">
                            ...
                            <div class="row">
                                <el-form-item :label="areaCode.length === 6 || areaCode.length === 4 ? '陪同人员:' : '信息员:'" prop="infoList">
                                    <el-select @change="blurCount" class="w200" v-model="ruleForm.infoList" multiple multiple-limit=5
                                        collapse-tags placeholder="请选择信息员">
                                        <el-option v-for="item in ruleForm.info" :key="item.id" :label="item.realName"
                    ...
                    <toggle-form title="巡查项">
                        <div v-if="patrolData.length>0" style="margin-top:35px;">
                            <el-table :key="index" v-for="(item,index) in patrolData" :class="index==0?'firsTeit':''"
                                :data="item.conList" :span-method="objectSpanMethod" border class="table-div brN">
                                ...
                                <el-table-column show-overflow-tooltip prop="remark" label="备注">
                                    <template slot-scope="scope">
                                        <el-input v-model="scope.row.remark" :maxlength="500" :disabled="!scope.row.qual"
                                            placeholder="备注前先选择是否检查通过"></el-input>
                                    </template>
                                </el-table-column>
                                <el-table-column align="center" label="附件">
                                    <template slot-scope="scope">
                                        <el-upload action="/api/upload" :before-upload="beforeUpPic" :on-success="picSuccess"
                                            :data="{type:'PATROL_RECORD',field:'PHOTO'}" :show-file-list="false">
                                            <el-button type="default" style="background:#F6F7F8;margin-right:20px;"
                                                @click="getRow(scope.row)">上传</el-button>
                                        </el-upload>
                                    </template>
                                </el-table-column>
                        ...
                        <div class="content">
                            ...
                            <div class="row" v-if="ruleForm.treat == 'TIME_LIMIT_RECTIFICATION'">
                                <el-form-item label="整改期限：" prop="rectifyTime">
                                    <el-date-picker class="w200" v-model="ruleForm.rectifyTime" type="datetime"
                                        :picker-options="pickerOptions" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择整改期限">
                                    </el-date-picker>
                                </el-form-item>
                            </div>
                            ...
                            <div class="row">
                                <el-form-item label="备注：" prop="">
                                    <el-input @blur="blurCount" class="w250" :maxlength="500" type="textarea"
                                        placeholder="如有需要请输入备注" :rows="4" v-model="ruleForm.treatRemark">
                            ...
                            <div class="row">
                                <el-form-item label="上传图片：" prop="">
                                    <el-upload :file-list="photo" action="/api/upload" list-type="picture-card"
                                        :on-success="uploadSuccess" :on-error="uploadError" :data="{type:'PATROL_RECORD',field:'PHOTO'}"
                            ...
                            <div class="row">
                                <el-form-item label="量化等级：" prop="">
                                    <dict-select @change="blurCount" v-model="ruleForm.quanLevel" dict="OTHER_QUANTIFY"
                                        placeholder="选择量化等级"></dict-select>
                                    <span style="font-size: 12px;color: #A2AAB6;">(选填)</span>
                                </el-form-item>
                            </div>
                            ...
                            <div class="row mRno">
                                ...
                                <el-upload :file-list="sign" v-if="ruleForm.isCooperate == 'yes'" action="/api/upload"
                                    list-type="picture-card" :on-success="uploadSuccess" :on-error="uploadError" :data="{type:'PATROL_RECORD',field:'SIGN'}"
                                    :before-upload="beforeAvatarUpload" :on-preview="handlePictureCardPreview"
        ...
        <div class="foot-btn" style="z-index:1">
            <div>
                <el-button style="width:120px;" @click="$router.push({path: '/biz/patrol/record/special', query:{pathName: '专项检查记录'}})">取消</el-button>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.areaCode = this.$store.getters.user.areaCode;
        },
        watch: {
