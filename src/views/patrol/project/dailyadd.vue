...
<template>
  <div>
            ...
            <div class="content">
              <div class="row">
                <el-form-item label="主体信息:" prop="">
                  <select-mainbody @slmainbody="slmainbody" :currentIndex='currentIndex' :apiFn='selectMainbodyApiFn'></select-mainbody>
                </el-form-item>
              </div>
              <!-- 选择的主体 -->
              ...
              <div class="row">
                <el-form-item label="巡查时间:" prop="patrolTime">
                  <div class="block">
                    <el-date-picker class="w200" @change="timeChange" v-model="ruleForm.patrolTime" type="datetime"
                                     value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择日期" :picker-options="pickerOptions0">
                    </el-date-picker>
                  </div>
                </el-form-item>
              </div>
              ...
              <div class="row">
                <el-form-item label="监管人员:" prop="patrolSoId">
                  <el-select @change="blurCount" class="w200" placeholder="请选择监管人员" v-model="ruleForm.patrolSoId">
                    <el-option :label="item.realName" :value="item.id" :key="item.id" v-for="item in ruleForm.soList" :disabled="item.disabled"></el-option>
                  </el-select>
                </el-form-item>
              </div>
              ...
              <div class="row">
                <el-form-item :label="infoTitle" prop="infoList">
                  <el-select @change="blurCount" class="w200" v-model="ruleForm.infoList" multiple-limit=5 multiple collapse-tags :placeholder="infoPlaceholder">
            ...
            <div class="content">
              ...
              <div class="row mRno">
                ...
                <el-upload :file-list="sign" v-if="ruleForm.isCooperate == 'yes'" action="/api/upload" list-type="picture-card" :on-success="uploadSuccess"
                  :on-error="uploadError" :data="{type:'PATROL_RECORD',field:'SIGN'}" :before-upload="beforeAvatarUpload" :on-preview="handlePictureCardPreview"
                  :on-remove="removePic" :limit="1">
                  <i class="el-icon-plus"></i>
                </el-upload>
                <div v-if="dialogVisible" class="modal" @click.stop="dialogVisible = false;">
                  <img :src="imageUrl" alt="">
    ...
    <div class="foot-btn" style="z-index:1">
      <div>
        <el-button style="width:120px;" @click="$router.push({path: '/biz/patrol/record/daily', query:{pathName: '日常监管记录'}})">取消</el-button>
...
<script>
  ...
  import {
    ...
    getBasicInfo,
    getDailyContext
  } from 'api/patrol/dailyRecordAdd.js';
  ...
  export default {
    ...
    data() {
      return {
        ...
        photo: [], //图片
        sign: [],  //电子签名
        selectMainbodyApiFn: dailyEntityPage, //选择主体的api方法
        ...
        infoArrayOfIdAndRealName:[],  //信息员数组。key：id  value：realName
        rules: {
          patrolTime: [
          ...
          infoList: [
            { required: true, message: '请选择信息员', trigger: 'change' },
            { validator: this.blurCount, trigger: 'change' }
          ],
          radioResult: [
    ...
    methods: {
        ...
        this.$refs['ruleForm'].validate((valid) => {
          if (valid) {
            this.$confirm('是否要提交巡查记录?', '提示', {
              ...
              let { entityId, patrolTime, score, result, treat, treatRemark, rectifyTime, isCooperate, patrolSoId, infoList, patrolPlanType } = this.ruleForm;
              score = score + "";
              obj.record = {
                entityId,
                patrolTime,
              ...
              //信息人员名称,多个逗号分隔
              obj.record.infoRealNames = this.getInfoRealNames(infoList);
      ...
      //信息人员名字
      getInfoRealNames(infoList) {
        let infoRealNames = "";
        infoList.forEach(v => {
          infoRealNames += this.infoArrayOfIdAndRealName[v] + ",";
        });
        return infoRealNames.substring(0, infoRealNames.length - 1);
      },
      ...
      slmainbody(obj) {
        ...
        if (!obj.id || !obj.type || !obj.entityType) return

        //获取基础信息
        getBasicInfo({ entityId: obj.id, type: obj.type }).then(res => {
          const data = res.data;
          ...
          this.blurCount();
          this.getInfoArrayOfIdAndRealName();
        })

        //获取巡查项
        getDailyContext({ type: 'SYS', entityId: obj.id })
          .then(res => {
            const data = res.data;
            if (!data) {
      ...
      //构造信息员列表数组，目的是保存的时候生成信息员姓名列表传给后端冗余，提高列表按信息员姓名搜索的效率（因为记录和信息员是一对多的关系）
      getInfoArrayOfIdAndRealName() {
        this.infoArrayOfIdAndRealName = [];   // k:id  v:realName
        this.ruleForm.info.forEach((v) => {
          this.infoArrayOfIdAndRealName[v.id] = v.realName.trim();
        });
      },
