...
<template>
  <div class="user-box">
    ...
    <div class="ipt-btn-box" style="justify-content: end;">
      ...
      <el-button class="selectHeight" type="primary" v-auth="'food.special.plan|func.add'" v-if="areaLevel != 'TOWN'" @click="add">添加计划
      </el-button>
    </div>
    <!--编辑弹窗-->
    ...
    <div class="tabOffTop170">
        <el-table :data="tableData" :border="true" :resizable="true"  class="table-div" @sort-change="handleSortChange"  v-loading="loading">
          <el-table-column type="index" width="50" label="序号"></el-table-column>
          <el-table-column show-overflow-tooltip prop="planName" min-width="180" label="计划名称"></el-table-column>
          <el-table-column show-overflow-tooltip prop="planNum" label="计划编号"  min-width="120"></el-table-column>
          <el-table-column show-overflow-tooltip prop="deptName" width="180" label="制定机构"></el-table-column>
          <el-table-column show-overflow-tooltip prop="startDate" v-if="areaLevel != 'CITY'" min-width="100" label="开始时间"></el-table-column>
          <el-table-column show-overflow-tooltip prop="endDate" v-if="areaLevel != 'CITY'" min-width="100" label="结束时间"></el-table-column>
          <el-table-column show-overflow-tooltip prop="bizType" resizable v-if="areaLevel != 'CITY'" min-width="100" label="主体类型"></el-table-column>
          <el-table-column show-overflow-tooltip prop="patrolNum" v-if="search.state != 'PEND_START'" min-width="100" label="已巡查主体数"></el-table-column>
          ...
          <el-table-column label="操作" min-width="200">
            <template slot-scope="scope">
              ...
              <a href="javascript:;" v-if="areaLevel != ' TOWN' && search.state == 'PEND_START' && search.state != 'ALREADY_COMPLETED'" @click="edit(scope.row)"  v-auth="'food.special.plan|func.edit'" class="operate-a">编辑</a>
              <a href="javascript:;" v-if="search.state == 'PEND_START' && areaLevel != 'TOWN' && scope.row.parentId == -1" class="operate-a" @click="deleteById(scope.row.id)">删除</a>
    ...
    async getPage() {
      ...
      plan.getPage(formData).then(r => {
        ...
        if(r.data.list.length > 0) {
          // 判断当前返回的结果是否与当前选中的tab一致, 一致则返回数据, 不一致则等待下一条请求返回的结果继续判断
          if(r.data.list[0].state === this.PlanStateList[this.mess.choose].name) {
            this.loading = false;
