  ...
  <div v-show="printShow" class="user-box"  id="content">
    <div class="printContent">
      ...
      <div id="printBody">
        ...
        <p>许可（备案）证号： <u> {{basicInfoData.licenseNumber}}</u>  </p>
        ...
        <p><span style="width:200px; display: inline-block;"> 检查人：<u> {{basicInfoData.patrolSoIdStr}}</u></span>
          <span style="width:200px; display: inline-block;" v-if='this.basicInfoData.areaCode'> {{ this.basicInfoData.areaCode.length == 6 ? '陪同人员':'信息员' }}：<u> {{messengers}} </u></span></p>
        <p>检查类别： <u> {{patrolType}} </u></p>
        <p>检查时间： <u> {{basicInfoData.patrolTime}}</u></p>
        <hr>
        <p class="text_indent">我们是<u> {{deptName}}</u>的执法人员 <u>{{basicInfoData.patrolSoIdStr}}、 {{messengers}}</u>，执法证件名称、编号是： ________________________ </p>
        ...
        <hr>
        <p class="text_indent" v-if="resultData.treat === 'TIME_LIMIT_RECTIFICATION'"> 针对上述问题，巡查结果为：<u>不符合</u>；处理方式为：<u>限期整改</u>。责令经营者务于 {{resultData.rectifyTime}} 前整改到位，并将整改情况反馈给 __________ （联系电话：__________ ）。 </p>
        <p class="text_indent" v-if="resultData.treat === 'ON_THE_SPOT_SPOT_RECTIFICATION'"> 针对上述问题，巡查结果为：<u>基本符合</u>；处理方式为：<u>现场整改</u>。 </p>
        <p class="text_indent" v-if="resultData.treat === 'NORMOUT_OF_BUSINESSAL'"> 针对上述问题，巡查结果为：<u>不符合</u>；处理方式为：<u>责令停业</u>。 </p>
        <p class="text_indent" v-if="resultData.treat === 'INVESTIGATION_AND_TREATMENT'"> 针对上述问题，巡查结果为：<u>不符合</u>；处理方式为：<u>调查处理</u>。 </p>
        <p class="text_indent" v-if="resultData.treat === 'NORMAL'">针对上述问题，巡查结果为：<u>符合</u>；处理方式为：<u>正常，无需处理</u>。 </p>
        ...
        <p v-if="resultData.sign" class="text_indent">电子签名：<u><img :src="pathFile + resultData.sign" alt=""></u></p>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      qrcode(id) {
        console.log(this.$store.getters.base.qrUrl,window)
        debugger
    ...
    computed: {
      spMessengers: function () {
        return this.messengers.substring(0, 50)
      }
    },
    data() {
      return {
