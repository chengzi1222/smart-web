  ...
  <div>
    ...
    <div class="user-box">
      <div class="ipt-btn-box" style="justify-content: end;">
        <el-input placeholder="姓名，处理人" class="w250 mr20 seekIpt" v-model="tableLimit.person"
          @keyup.enter.native="changeSearch">
          <i slot="suffix" class="el-input__icon el-icon-search" @click="changeSearch"></i>
        </el-input>
        <el-select class="mr20" v-model="tableLimit.handleStatus" @change="changeSearch" placeholder="处理状态" clearable>
          <el-option v-for="item in statusOptions" :key="item.value" :label="item.label" :value="item.value">
          </el-option>
        </el-select>
        <el-select class="mr20" v-model="tableLimit.handleType" @change="changeSearch" placeholder="处理方式" clearable>
          <el-option v-for="(value,key) in methodOptions" :key="value" :label="value" :value="key">
          </el-option>
        </el-select>
      ...
      <div class="ipt-btn-box" style="justify-content: end;">
        <div style="display: inline-block;">
          预警日期：
          <el-date-picker :picker-options="pickerOptions0" v-model="tableLimit.startDate" @change="changeSearch"
      ...
      <div class="tabOffTop120">
        <el-table v-loading="loading" element-loading-text="数据正在努力加载中" :border="true" :resizable="true"
          ...
          <el-table-column show-overflow-tooltip min-width="150" label="健康证预览">
            <template slot-scope="scope">
              <el-image v-if="scope.row.healthPreview !==null&&scope.row.healthPreview !==''"
                style="width: 100px; height: 100px" :src="pathFile+scope.row.healthPreview"
                :preview-src-list="[pathFile+scope.row.healthPreview]">
              </el-image>
            </template>
          </el-table-column>
          <el-table-column show-overflow-tooltip prop="realName" min-width="150" label="姓名"></el-table-column>
          ...
          <el-table-column show-overflow-tooltip min-width="100" label="处理状态">
            <template slot-scope="scope">{{statusOptions_[scope.row.handleStatus*1]}}</template>
          </el-table-column>
    ...
    <Dialog :dialogData="dialogData" url="/ewcenter/ewHealth/healthHandle" :showTip="disposeAddDialog"
      v-if="disposeAddDialog" @refresh="disposeRefresh" disposeType='02' @cancel="closeDialog">
    </Dialog>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async searchPage() {
        this.loading = true;
        let result = await api.getEwHealthList(this.tableLimit);
        if (result.status) {
          this.tableData = [];
