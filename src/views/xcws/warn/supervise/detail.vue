    ...
    <div>
        ...
        <div class="user-box">
            <div class="ipt-btn-box" style="justify-content: end;">
                ...
                <el-select class="mr20" v-model="tableLimit.patrolType" @change="searchTab" placeholder="请选择检查类型"
                           clearable>
                    <el-option
            ...
            <div class="ipt-btn-box" style="justify-content: end;">
                ...
                <el-date-picker @change="searchTab" value-format="yyyy-MM-dd" class=" w140 mr10" type="date"
                                v-model="tableLimit.startDate" clearable
                                placeholder="选择开始日期" :picker-options="beforeDate"></el-date-picker>
                <span>至</span>
                <el-date-picker @change="searchTab" value-format="yyyy-MM-dd" v-model="tableLimit.endDate"
                                class=" w140 ml10 mr20" type="date" clearable
                                placeholder="选择截止日期" :picker-options="afterDate"></el-date-picker>
            ...
            <div class="tabOffTop120">
                <el-table v-loading="loading" element-loading-text="数据正在努力加载中" :border="true" :resizable="true"
                    ...
                    <el-table-column show-overflow-tooltip prop="patrolName" min-width="120"
                                     label="检查任务名称"></el-table-column>
                    <el-table-column show-overflow-tooltip min-width="100" prop="importantCnt" label="不合格重点项数">
                    </el-table-column>
                    <el-table-column show-overflow-tooltip prop="normalCnt" min-width="100"
                                     label="不合格一般项数"></el-table-column>
                    <el-table-column show-overflow-tooltip prop="patrolTime" min-width="100"
                                     label="检查时间"></el-table-column>
                    <el-table-column show-overflow-tooltip prop="ewDate" min-width="120" label="预警时间"
                    ...
                    <el-table-column label="操作" min-width="150" fixed="right">
                        <template slot-scope="scope">
                            ...
                            <a href="javascript:;" class="operate-a"
                               @click="goDetail(scope.row.patrolId,scope.row.type)">检查记录</a>
                        </template>
                    </el-table-column>
                </el-table>
...
<script>
    ...
    export default {
        ...
        async mounted() {
            ...
            this.queryLimit.entityId = this.$route.query.entityId ||  this.$route.params.entityId
            this.tableLimit.entityId = this.$route.query.entityId || this.$route.params.entityId
        ...
        methods: {
            ...
            async searchPage() {
                this.loading = true;
                let result = await guard.getSupervise(this.tableLimit)
                if (result.status) {
                    this.tableData=[];
            ...
            goDetail(id, type) {
                let url = ''
                let data = {}
                if (type == '030101' || type == '030102') {
                    url = 'patrol.repatrol.dailyDetail'
                    data = type == '030101' ? {'recordId': id, 'patrolType': 'DAILY'} : {
                        'recordId': id,
                        'patrolType': 'SPECIAL'
                    }
                } else if (type == '030103') {
                    url = 'random.task.record.detail'
                    data = {'recordId': id}
                } else if (type == '030104') {
                    this.$router.push({name: 'patrol.repatrol.ad', params: {'curd': 'detail', 'repatrolRecordId': id, 'page':'warn'}})
                }
                this.$router.push({
                    name: url,
                    query: data
