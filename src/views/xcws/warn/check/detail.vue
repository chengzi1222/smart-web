    ...
    <div>
        ...
        <div class="user-box">
            <div class="ipt-btn-box" style="justify-content: end;">
                <el-input placeholder="处理人" class="w200 mr20 seekIpt" v-model="tableLimit.keyword"
                          @keyup.enter.native="handleIconClick">
                    <i slot="suffix" class="el-input__icon el-icon-search" @click="handleIconClick"></i>
                </el-input>
                <el-select class="mr20" v-model="tableLimit.handleStatus" @change="searchTab" placeholder="请选择处理状态" clearable>
                    <el-option v-for="item in statusOptions" :key="item.value" :label="item.label" :value="item.value"></el-option>
                </el-select>
                <el-select class="mr20" v-model="tableLimit.handleType" @change="searchTab" placeholder="请选择处理方式" clearable>
                    <el-option v-for="(value,key) in handleType" :key="value" :label="value" :value="key"></el-option>
                </el-select>
                <el-select class="mr20 w200" v-model="tableLimit.handleDept" @change="searchTab" placeholder="处理部门" clearable>
                    <el-option v-for="(value,key) in handleDept" :key="value" :label="value" :value="key"></el-option>
                </el-select>
                <el-select class="mr20 w200" v-model="tableLimit.ewType" @change="searchTab" placeholder="异常类型" clearable>
                    <el-option v-for="item in ewType" :key="item.value" :label="item.label" :value="item.value"></el-option>
                </el-select>
            </div>
            <div>
            ...
            <div class="tabOffTop120 mt20">
                <el-table v-loading="loading" element-loading-text="数据正在努力加载中" :border="true" :resizable="true"
                    ...
                    <el-table-column show-overflow-tooltip prop="pdtName" min-width="190" label="检测异常类型">
                        <template slot-scope="scope">{{warning[scope.row.type]}}</template>
                    </el-table-column>
                    <el-table-column show-overflow-tooltip prop="detectWay" min-width="120" label="检测方式"></el-table-column>
                    <el-table-column show-overflow-tooltip prop="deliveDate" min-width="100" label="检测异常日期"></el-table-column>
                    <el-table-column show-overflow-tooltip prop="detCnt" min-width="100" label="当日配送商品检测批次数"></el-table-column>
                    <el-table-column show-overflow-tooltip prop="shouldDetCnt" min-width="100" label="当日配送商品应检测批次数"></el-table-column>
                    <el-table-column show-overflow-tooltip prop="detRate" min-width="100" label="当日配送商品检测率"></el-table-column>
                    <el-table-column show-overflow-tooltip prop="unquaCnt" min-width="100" label="当日检测不合格数" sortable='custom'></el-table-column>
                    ...
                    <el-table-column show-overflow-tooltip min-width="100" label="处理状态">
                        <template slot-scope="scope">{{scope.row.handleStatus == 1? '已处理': '未处理'}}</template>
                    </el-table-column>
                    ...
                    <el-table-column show-overflow-tooltip min-width="100" label="处理部门">
                        <template slot-scope="scope">{{handleDept[scope.row.handleDept]}}</template>
                    </el-table-column>
                    ...
                    <el-table-column show-overflow-tooltip min-width="100" prop="handleRemark" label="处理备注"></el-table-column>
        ...
        <Dialog :dialogData="dialogData"
                url="/ewcenter/ewDetect/ewhandle"
    ...
    export default {
        ...
        data() {
            return {
                ...
                handleType: getMapping().handle,
                handleDept: getMapping().disposeDept,
                ewType: [{
                    value: "020101",
                    label: "检测超标"
                },{
        ...
        methods: {
            ...
            async searchPage() {
                this.loading = true;
                let result = await guard.getCheck(this.tableLimit)
                if (result.status) {
                    this.tableData = [];
