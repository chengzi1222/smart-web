          ...
          <div class="content">
            ...
            <div class="row">
              <el-form-item label="主营项目:" prop="projectMain">
                ...
                <p style="color: #ff4949" v-if="cfda.projectMain.length == 0">请至少选择一项</p>

              </el-form-item>
            </div>
            <div class="row">
              <el-form-item label="兼营项目:">
                <el-checkbox-group @change="blurCount" v-model="cfda.projectSid" size="large">
...
<script>
  ...
  import {addAutoSave,addBackup,getEntityByCredit,getGrid,getInfoAssi,getSoEmp,getSyj,removeFile} from "api/cfda/add.js";
  ...
  async mounted() {
    ...
    methods: {
      async init() {
        ...
        //经营业态
        listDictUserItemByDictCode("CFDA_S_SUB").then((r) => {
        ...
        listDictUserItemByCodeAll("OTHER_MANAGEMENT").then((r) => {
          ...
          console.log(this.detailedEngageItem)
        });

        //主营项目
        listDictUserItemByDictCode("CFDA_S_SIDELINE").then((r) => {
        ...
        //食品类别
        listDictUserItemByDictCode("CFDA_S_MAIN").then((r) => {
          let list1 = r.data;
          let list2 = [];
        ...
        //主体备案有效期
        listDictUserItemByDictCode("CFDA_S_PERIOD").then((r) => {
        ...
        getCfda(this.entityId).then((r) => {
          ...
          for (let i = 0; i < r.data.projectMain.length; i++) {
            let fs = r.data.projectMain[i];
          ...
          // r.data.licLimit = new Date(r.data.licLimit);
...
//      if (this.cfda.quanLevel != '') cnt1++;
      ...
      if (this.cfda.projectMain.length > 0) cnt1++;
      if (this.cfda.projectSid.length > 0) cnt1++;
