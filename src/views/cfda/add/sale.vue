          ...
          <div class="content">
            ...
            <div class="row">
              <el-form-item label="主营项目:" prop="projectMain">
                ...
                <p style="color: #ff4949;font-size: 12px;" v-if="isBtnSelect && cfda.projectMain.length == 0">请至少选择一项</p>
              </el-form-item>
            </div>
            <div class="row">
...
import { addCfda, addAutoSave, getSyj, getGrid, getSoEmp, getInfoAssi, getAutoSave, addBackup, getBackup, removeFile, getEntityByCredit } from "api/cfda/add.js";
...
export default {
  ...
  methods: {
    ...
    addCfda() {
      ...
      this.$refs['cfda'].validate((valid) => {
        this.flag = false;
        if (valid) {
          if (this.cfda.projectMain.length == 0) {
            this.isBtnSelect = true;
            return;
          }
  ...
  data() {
    var fileValid = (rule, value, callback) => {
      if (this.cfda.fileList.length > 0) {
        callback();
      } else {
    ...
    return {
      ...
      rules: {
        ...
        fileList: [
          { validator: fileValid, trigger: 'blur' }
        ]
