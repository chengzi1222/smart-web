      ...
      <el-form :model="cfda" :rules="rules" ref="cfda" label-width="110px" class="demo-ruleForm labW150">
          ...
          <div class="content">
            ...
            <div class="row">
              <el-form-item label="三小备案时间:" prop="recordBegin">
                <el-date-picker class="w140" v-model="cfda.recordBegin" :editable="false" type="date" placeholder="选择日期范围">
                </el-date-picker>
              </el-form-item>
            </div>
          ...
          <div class="content">
            ...
            <div class="row">
              <el-form-item label="身份证号:" prop="idNo">
                <el-input class="w300" :maxlength="18" v-model="cfda.idNo" placeholder="请输入18位身份证号"></el-input>
              </el-form-item>
            </div>
            <div class="row">
        ...
        <toggle-form title="场地信息">
          <div class="content">
            ...
            <div class="row red">
              <el-form-item label="(经营场所)证照地址:" prop="spaceAddr">
        ...
        <toggle-form title="人员信息">
          ...
          <el-table :data="cfda.emps" style="width: 100%" v-if="cfda.emps && cfda.emps.length > 0">
            ...
            <el-table-column label="备注">
              <template slot-scope="scope">
                <el-input class="w200" v-model="scope.row.remark" placeholder="请输入备注（非必填）" :maxlength="255"></el-input>
              </template>
            </el-table-column>
        ...
        <toggle-form title="附件上传">
          <attachment-upload @delete="handleRemoveFile" @uploadChange="uploadChange" style="margin-top: 20px;" :options="options" v-model="updated"></attachment-upload>
        </toggle-form>
      </el-form>
    </div>
...
<script>
  ...
  import { addCfda, addAutoSave, getSyj, getGrid, getSoEmp, getInfoAssi, getAutoSave, addBackup, getBackup, removeFile, getEntityByCredit,getArea } from "api/cfda/add.js";
  ...
  export default {
    ...
    async mounted() {
      ...
      if (this.$route.query.edit) {//编辑进入
        ...
        getBackup(this.$route.query.edit).then((r) => {
          ...
          data.infoAssi_a = data.infoAssi;
          data.grid_a = data.grid;
          this.cfda = r.data;
        ...
        if (this.$route.query.auto) {
          getAutoSave().then((results) => {
            results.data.infoAssi_a = results.data.infoAssi;
            results.data.grid_a = results.data.grid;
            this.cfda = results.data;
            this.detailedEngage = results.detailedEngage;
            this.getGridChange(this.cfda.syj)
            // 附件
            this.handleFile(results.data.fileList);
            this.$nextTick(() => {
            this.flag=false;
              // this.addAutoSave()
            });
          })
        } else {//自动保存过然后跳具体的那一项带参数auto=true
          getAutoSave().then((results) => {
            let data = results.data;
            if (data.createData!==undefined || data) {
              this.$confirm('检测到您' + data.createData + '有没有完成的录入信息，是否继续上次的录入操作？', '录入提示', {
                ...
                this.detailedEngage = data.detailedEngage;

              }).catch(_ => {
                if(this.syjSelect[0]&&this.syjSelect.length>0){
                  this.cfda.syj=this.syjSelect[0].id
                  this.getGridChange(this.cfda.syj)
                }
               });
            }else{
    ...
    methods: {
      ...
      addAutoSave() {
        if(this.flag==false && this.cfda.percentageNumber > 40){
...
//        licStateSelect: [],
        ...
        rules: {
          ...
          recordBegin: [
            { required: true, message: '请选择主体备案日期', trigger: 'blur' },
            { validator: this.blurCount, trigger: 'blur' }
          ],
          corpName: [
          ...
          spaceDst: [
            ...
            //            { required: true, message: '请选择场所分布', trigger: 'change' },
            { validator: this.blurCount, trigger: 'change' }
          ],
          spaceArea: [
