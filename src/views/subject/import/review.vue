<template>
  <div class="user-box">
    <div class="ipt-btn-box clear_a">
      <el-input placeholder="许可证编号、主体名称、社会信用代码" class="w250 seekIpt" v-model="searchMap.keysWork" @keyup.enter.native="">
        <i slot="suffix" class="el-input__icon el-icon-search" @click="pageSearch"></i>
      </el-input>
      <enum-select class="selectHeight" multiple v-model="searchMap.licState" placeholder="许可证状态" enum="com.ybveg.govx.enums.LicenceStateEnum"
        @change="pageSearch"></enum-select>
      <enum-select class="selectHeight" multiple v-model="searchMap.licenceType" placeholder="许可证类型" :enum="licenceTypeEnum" @change="pageSearch"></enum-select>
      <select-area-grid class="selectHeight" placeholder="所属区域" @change="areaCodeChange"></select-area-grid>

       <el-select v-if="entityType == 'restaurant'" collapse-tags multiple clearable class="selectHeight" v-model="searchMap.superviseType" placeholder="请选择监管类型" @change="pageSearch">
        <el-option v-for="item in superviseTypeList" :key="item.key" :label="item.name" :value="item.key">
        </el-option>
      </el-select>

      <el-select v-if="entityType == 'restaurant'" collapse-tags multiple clearable class="selectHeight" v-model="searchMap.networkOperation" placeholder="请选择网络经营类型" @change="pageSearch">
    ...
    <div class="tabOffTop120">
      <el-table @sort-change="sortChange" :border="true" :resizable="true" :data="tableData" @selection-change="selectChange" class="table-div">
        ...
        <el-table-column show-overflow-tooltip prop="licenceType" min-width="120" label="*许可证类型"></el-table-column>
        <el-table-column show-overflow-tooltip prop="licState" min-width="120" label="*许可证状态"></el-table-column>
        <el-table-column show-overflow-tooltip prop="creditCode" min-width="100" label="社会统一信用代码"></el-table-column>
        <el-table-column show-overflow-tooltip prop="spaceAddr" min-width="100" label="*经营地址"></el-table-column>
        ...
        <el-table-column v-if="entityType.toUpperCase() == 'RESTAURANT'" show-overflow-tooltip prop="superviseType" min-width="100" label="监管类型"></el-table-column>
        <el-table-column v-if="entityType.toUpperCase() == 'RESTAURANT'" show-overflow-tooltip prop="networkOperation" min-width="100" label="网络经营"></el-table-column>
        <el-table-column show-overflow-tooltip prop="manageState" min-width="100" label="营业状态"></el-table-column>
        <el-table-column show-overflow-tooltip prop="createTime" min-width="100" sortable='custom' label="创建时间"></el-table-column>
      ...
      <div class="pagination-box">
        <el-pagination background :current-page="page.pageNum" :page-sizes="[10, 50, 100, 300, 500]" :page-size="page.pageSize" layout="total, sizes, prev, pager, next, jumper"
...
<script>
  ...
  import { page, review, del } from 'api/sub/import'
  import { getLicenceTypeEnum } from './importUtils'
  ...
  export default {
    ...
    mounted() {
      ...
      this.licenceTypeEnum = getLicenceTypeEnum('FOOD', this.searchMap.entityType)
      if (this.entityType.toUpperCase() == 'RESTAURANT') {
