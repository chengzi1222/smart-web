    ...
    <div class="rightBox">
        ...
        <div class="infoBox clear_a" v-loading="loading" element-loading-text="数据正在努力加载中">
            ...
            <div class="base baseStyle">
                <p class="innerBoxTitie"><span class="line"></span>培训记录详情</p>
                <div class="baseInfo overHide">
                    <div class="clear_a mb14">
                        <div style="width:70px;" class="col66">培训名称：</div>
                        <div style="width:calc(100% - 70px);">{{info.eduName}}</div>
                    </div>
                    <p class="textell"><span class="col66">培训日期：</span>{{info.eduDate}} {{info.eduTime}}</p>
                    <p><span class="col66">培训人数：</span>{{info.personCnt}}</p>
                    <div class="clear_a mb14">
                        <div style="width:60px;" class="col66">主讲人：</div>
                        <div style="width:calc(100% - 60px);">{{info.lector}}</div>
                    </div>
                    <p class="textell"><span class="col66">单位职务：</span>{{info.lectorPost}}</p>
                    <p><span class="col66">时长：</span>{{info.hourCnt}}</p>
                    <div class="clear_a mb14">
                        <div style="width:48px;" class="col66">地点：</div>
                        <div style="width:calc(100% - 48px);">{{info.address}}</div>
                    </div>
                    <div class="clear_a">
                        <div style="width:70px;" class="col66">培训内容：</div>
                        <div style="width:calc(100% - 70px);">{{info.remark?info.remark:'暂无培训内容'}}</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    export default {
        methods: {
            ...
            getInfo() {
                ...
                this.loading = true;
                api.getTrainingInfo(obj).then(r => {
                    if (r.code == 200) {
                        this.loading = false;
                        this.info = r.data;
                    }
