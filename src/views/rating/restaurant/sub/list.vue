<template>
  <div class="bigbox">
    ...
    <div class="mt20 mb20 clear_a">
      <el-input placeholder="主体名称, 备案号, 社会信用代码" class="w250 seekIpt mr20" v-model="queryLimit.search"
        @keyup.enter.native="searchTab" :maxlength="30">
      ...
      <el-select class="w200 mr20" v-model="queryLimit.entityType" placeholder="主体类型" clearable @change="searchTab">
        <el-option v-for="item in businessType" :key="item.key" :label="item.name" :value="item.key"></el-option>
      </el-select>
      <el-select class="w200 mr20" v-model="searchKey.subType" placeholder="主体业态" clearable collapse-tags multiple
        v-if="queryLimit.entityType !== 'CFDARESTAURANT'" @change="multipleSelect('subType')">
    ...
    <div class="mt20 mb20 clear_a">
      <el-select class="w200 mr20" v-model="searchKey.superviseType" clearable collapse-tags multiple placeholder="监管类型"
        v-if="queryLimit.entityType !== 'CFDARESTAURANT'" @change="multipleSelect('superviseType')">
        <el-option v-for="item in  supervisionType" :key="item.key" :label="item.name" :value="item.key"></el-option>
      </el-select>
      <el-select class="w200 mr20" v-model="searchKey.riskLevel" clearable collapse-tags multiple placeholder="风险等级"
      ...
      <div class="ml20">
        ...
        <el-date-picker @change="handleClick" value-format="yyyy-MM-dd" class="w140 mr10" type="date"
          v-model="queryLimit.startDate" clearable placeholder="选择开始日期" :picker-options="beforeDate"></el-date-picker>
        <span>至</span>
        <el-date-picker @change="handleClick" value-format="yyyy-MM-dd" v-model="queryLimit.endDate"
          class="w140 ml10 mr20" type="date" clearable placeholder="选择截止日期" :picker-options="afterDate">
        </el-date-picker>
      </div>
      <el-select class="w200 mr20" v-model="searchKey.status" clearable collapse-tags multiple placeholder="状态"
        @change="multipleSelect('status')">
        <el-option v-for="item in status" :key="item.value" :label="item.label" :value="item.value"></el-option>
      </el-select>
    </div>
    <div>
      <el-table v-loading="loading" element-loading-text="数据正在努力加载中" :border="true" :resizable="true" :data="tableData"
        class="table-div">
        ...
        <el-table-column show-overflow-tooltip prop="entityType" min-width="100" label="主体类型">
          <template slot-scope="scope">{{scope.row.entityType === 'CFDARESTAURANT'?'小经营店（餐饮)':'餐饮主体'}}</template>
        </el-table-column>
        <el-table-column show-overflow-tooltip prop="subTypeStr" min-width="100" label="主体业态"
          v-if="queryLimit.entityType !== 'CFDARESTAURANT'"></el-table-column>
        <el-table-column show-overflow-tooltip prop="superviseTypeStr" min-width="100" label="监管类型"
          v-if="queryLimit.entityType !== 'CFDARESTAURANT'"></el-table-column>
    ...
    <adjust-dialog :showTip="showTip" :listData="listData" @cancel="showTip =false" @success="adjustSuccess"
      v-if="showTip"></adjust-dialog>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleClick() {
        // if (this.queryLimit.startDate && this.queryLimit.endDate || this.queryLimit.startDate === null && this
        //   .queryLimit.endDate === null) {
          this.searchTab();
        // }
      },
      searchTab() {
