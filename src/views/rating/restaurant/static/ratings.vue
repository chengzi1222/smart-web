<template>
  <div v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading"
    ...
    <div style="margin-top:20px;border-left:3px solid  #0DB5EF;">
      <span style="margin-left:10px;font-size: 16px;color: #393939;font-weight:bolder;">静态评分</span>
    </div>
    <div class="container">
      <el-form :model="repatrol" :rules="rules" ref="repatrol" label-width="140px" class="demo-ruleForm">
        ...
        <toggle-form title="主体情况">
          <div class="content">
            <div class="row rowDjpd" v-if="baseInfo.entityType=='RESTAURANT'">
              <el-form-item label="经营场所面积:">
                <span>{{baseInfo.spaceArea ? baseInfo.spaceArea  : '暂无'}}</span>
              </el-form-item>
            </div>
            <div class="row rowDjpd">
              <el-form-item label="制售食品种类:">
                <span
                  v-if="baseInfo.entityType=='RESTAURANT'">{{baseInfo.subOperProStr ? baseInfo.subOperProStr  : '暂无'}}</span>
                <span v-else>{{baseInfo.cfdaOperProStr ? baseInfo.cfdaOperProStr  : '暂无'}}</span>
              </el-form-item>
            </div>
            <div class="row rowDjpd" v-if="baseInfo.entityType=='RESTAURANT'">
            ...
            <div class="row rowDjpd" v-if="baseInfo.entityType!='RESTAURANT'">
              <el-form-item label="选址:">
                <span> {{baseInfo.cfdaRealityAddr ? baseInfo.cfdaRealityAddr  : '暂无'}}</span>
              </el-form-item>
            </div>
          </div>
        </toggle-form>
            ...
            <div class="content">
              <div class="row  bottomContent">
                <el-form-item label="分数统计选项:">
                  {{baseInfo.statisticsOptions=='YES'?'是':'否'}}
                </el-form-item>
              </div>
              <div class="row  bottomContent">
                <el-form-item label="备注:">
                  {{baseInfo.tableRemark?baseInfo.tableRemark:'暂无'}}
                </el-form-item>
              </div>
            </div>
          </el-form>
    ...
    <div class="imgNameBox" :style="imgNameStyle" v-if="accessoryListIf" @mouseenter="accessoryListIf = true"
      ...
      <div>
        <p v-for="(item,index) in itemPicList" :key="item.picSrc">
          <span @click.stop="previewImg(itemPicList,index,'检查项附件')">
        ...
        this.$refs['repatrol'].validate((valid) => {
          ...
          if (valid) {
            this.$confirm('是否要提交静态评分?', '提示', {
              ...
              this.repatrol.statisticsOptions = this.baseInfo.statisticsOptions;
...
<style lang='scss'>
  ...
  .rowDjpd .el-form-item .el-form-item__content>span {
    max-width: 550px;
  }
</style>