  ...
  <div class="bigbox">
    ...
    <div class="mt20 mb20 clear_a">
      ...
      >
        ...
        <el-select
        class="selectHeight"
        v-model="queryLimit.type"
        :clearable="true"
        placeholder="类型"
        @change="searchTab"
      >
        <el-option v-for="item in type" :key="item.key" :label="item.name" :value="item.key"></el-option>
      </el-select>
      <el-select
        class="selectHeight"
      ...
      <el-button
        ...
        type="primary"
        icon="el-icon-plus"
        @click="go('restaurantRating.pointManage.add')"
      >添加</el-button>
    </div>
    <div>
      <el-table
      ...
      >
        ...
        <el-table-column show-overflow-tooltip  min-width="100" label="类型">
          <template slot-scope="scope">
                {{scope.row.type === 'DYNAMIC'?'动态评分表':'静态评分表'}}
            </template>
        </el-table-column>
        ...
        <el-table-column label="操作" min-width="150" fixed="right">
          <template slot-scope="scope">
            <a href="javascript:;" class="operate-a" @click="go('restaurantRating.pointManage.edit',scope.row.id)">编辑</a>
            <a href="javascript:;" class="operate-a" @click="go('restaurantRating.pointManage.detail',scope.row.id)">详情</a>
            <a href="javascript:;" class="operate-a" @click="operate(scope.row)">{{scope.row.status === 'ENABLE'? '禁用':'启用'}}</a>
            <a href="javascript:;" class="operate-a" v-if="scope.row.status === 'DISABLE'" @click="del(scope.row.id)">删除</a>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    async operate(row){
       ...
       this.$confirm(`是否确认${text}该巡查要点表`, '提示', {
    ...
    async del(id){
       this.$confirm('是否确认删除该表，删除后无法使用，请谨慎操作！', '提示', {
          ...
          let result = await api.delTable(id);
          if(result.status){
            this.$message({
            type: 'success',
            message: '删除成功'
