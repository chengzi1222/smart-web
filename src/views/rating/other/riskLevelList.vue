  ...
  <div class="user-box">
    ...
    <div class="main-body no-star">
      <div class="form-item">主体类型:
        ...
        <el-button class="entity-tap" v-for="(item,index) in btns" :key="index" @click="handleBtn(item.entityParentType,index)" :class="{active:index==defaultIndex}">{{item.title}}</el-button>
    ...
    <div class="ipt-btn-box" style="justify-content: end;">
      <el-input placeholder="主体名称、许可备案号" class="w250 seekIpt" v-model="search.entityName" @keyup.enter.native="handleIconClick">
        <i slot="suffix" class="el-input__icon el-icon-search" @click="handleIconClick"></i>
      </el-input>

      <el-select clearable class="selectHeight w250" v-model="search.riskLevelThis" placeholder="风险等级（本年）" @change="opSearch">
        <el-option v-for="item in riskLevelList" :key="item.id" :label="item.nameOR" :value="item.id">
        </el-option>
      </el-select>

      <el-select clearable class="selectHeight w250" v-model="search.riskLevelNext" placeholder="风险等级（明年）" @change="opSearch">
        <el-option v-for="item in riskLevelList" :key="item.id" :label="item.nameOR" :value="item.id">
        </el-option>
      </el-select>

      <el-cascader class="selectHeight w250" :clearable="true" v-model="entityAreaCodeList" placeholder="主体所属区域" :change-on-select="true"
        :options="areaList.options" :props="areaList.props" :show-all-levels="false" @change="handleAreaChange">
      </el-cascader>

      <el-select clearable class="selectHeight w250" v-model="search.entityType" placeholder="主体类型" v-if="entityParentType == 'CFDA'"
        @change="opSearch">
        <el-option v-for="item in entityTypeList" :key="item.key" :label="item.name" :value="item.key">
        </el-option>
      </el-select>

      <el-select clearable class="selectHeight w250" v-model="search.licenseStatus" placeholder="证照状态（全部）" v-if="entityParentType != 'CFDA'"
        @change="opSearch">
    ...
    <div class="tabOffTop170">
      <el-table :data="tableData" class="table-div" @sort-change="handleSortChange" v-loading="loading">
        ...
        <el-table-column :show-overflow-tooltip="true" prop="entityName" label="主体名称"></el-table-column>
        <el-table-column show-overflow-tooltip prop="licenseNumber" label="许可备案号"></el-table-column>
        <el-table-column :show-overflow-tooltip="true" prop="realityAddr" label="实际经营地址"></el-table-column>
        <el-table-column show-overflow-tooltip prop="areaName" label="所属区域"></el-table-column>
        <el-table-column show-overflow-tooltip prop="riskLevelThis" label="风险等级（本年）"></el-table-column>
        <el-table-column show-overflow-tooltip prop="riskLevelNext" label="风险等级（明年）"></el-table-column>
        <el-table-column @cell-mouse-enter="handleRegulationRecord" label="监管记录">
          <template slot-scope="scope">
            <span @mouseenter="takeMove(scope.row)" @mouseleave="takeLeve" style="overflow: hidden;white-space: nowrap;width: 109px;text-overflow: ellipsis;">
              当前量化等级：{{scope.row.quantifyLevelThis}}；当前风险等级：{{scope.row.riskLevelThis}}； 行政处罚：{{scope.row.punishNumber}}；刑事案件：{{scope.row.criminalCaseNumber}}；
              动态评级：
        ...
        <el-table-column show-overflow-tooltip prop="riskAssessTime" sortable="custom" label="最新评定时间"></el-table-column>
        <el-table-column width="250" label="操作">
          <template slot-scope="scope">
            <a href="javascript:;" @click="firstAssess(scope.row,'ANNUAL')" class="operate-a">评定</a>
            <a href="javascript:;" v-auth="'food.biz.cb,food.biz.pb,food.biz.rb,food.biz.cfda'" @click="ifType(scope.row)" class="operate-a">主体档案</a>
    ...
    <popupBody v-if="evaluate" :rules="rules">
      ...
      <el-select class="w130" style="margin-right: 10px;" v-model="grade" placeholder="请选择">
        <el-option v-for="item in riskLevelList" :key="item.id" :label="item.nameOR" :value="item.id" :disabled="item.id === 'NOASSESS'">
        </el-option>
      </el-select>
...
<script>
  ...
  import { listDict, listEntityType } from "api/enforce/punish";
  ...
  export default {
    ...
    methods: {
      ...
      handleBtn(entityType, index) {
        this.$router.push({
          query: {
            entityParentType: entityType,
            pathName: "静态评定"
          }
        })
        this.entityParentType = entityType;
        this.defaultIndex = index;
        this.search.entityParentType = entityType;
        this.listRiskLevel();
      },
      ...
      assess(row) {
        ...
        rating.listRiskLevelAssessRecord(obj).then((d) => {
