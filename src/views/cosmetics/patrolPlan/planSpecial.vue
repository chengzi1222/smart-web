...
<template>
  <div class="user-box">
    ...
    <div class="ipt-btn-box" style="justify-content: end;">
      ...
      <enum-select  class="w200" v-if="areaLevel != 'CITY'"  @change="handleIconClick" v-model="search.bizType" enum="com.ybveg.govx.enums.cosmetic.CosmeticEntityTypeEnum" placeholder="主体类型（全部）"></enum-select>
    ...
    <div class="tabOffTop170">
      <el-table  v-loading="loading" element-loading-text="数据正在努力加载中"   :border="true" :resizable="true"  :data="tableData" class="table-div" @sort-change="handleSortChange">
        ...
        <el-table-column show-overflow-tooltip prop="start_date" v-if="areaLevel != 'CITY'" width="100" label="开始时间"></el-table-column>
        <el-table-column show-overflow-tooltip prop="end_date" v-if="areaLevel != 'CITY'" width="100" label="结束时间"></el-table-column>
        <el-table-column show-overflow-tooltip prop="biz_type_name" resizable v-if="areaLevel != 'CITY'" label="主体类型"></el-table-column>
        ...
        <el-table-column label="操作" min-width="200">
          <template slot-scope="scope">
            ...
            <a href="javascript:;" v-if="search.state == 'EXECUTING' && areaLevel == 'COUNTY'"  @click="complete(scope.row)" class="operate-a">完成</a>
            <a href="javascript:;" v-if="areaLevel != ' TOWN' && search.state == 'PEND_START' || areaLevel == 'COUNTY' && search.state != 'ALREADY_COMPLETED'" @click="edit(scope.row)"  class="operate-a">编辑</a>
            <a href="javascript:;" v-if="search.state == 'PEND_START' && areaLevel != 'TOWN' && scope.row.parent_id == -1" class="operate-a" @click="deleteById(scope.row.id)">删除</a>
  ...
  methods: {
    ...
    complete(row){//完成按钮
     this.$confirm('是否确认该计划已完成？', '提示', {
          ...
          let formdata=new FormData();
          formdata.append('planId', row.id);
          api.complete(formdata).then(d=>{
            if(d.status){
               this.$message({
                  message: '已修改为完成状态！',
                  type: 'success'
