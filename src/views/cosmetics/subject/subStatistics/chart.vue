    ...
    <ul class="flex-box mt20" v-if="dataLis">
      ...
      <div class="mt20">
        <el-select style="vertical-align: top;" class="w150 mr20" v-model="riskRankValue" placeholder="请选择状态" @change="getParticular">
          ...
          <el-option :label="'未评估'" :value="'NOASSESS'"></el-option>
        </el-select>
        ...
        <el-select class="w150 mr20" v-if="subType!='COSMETIC_PRODUCT'" v-model="qiyeTypeValue" @change="getParticular" placeholder="企业形态" clearable>
      ...
      <div class="way-box mt20">
        <div class="way-div"  v-if="subType!='COSMETIC_PRODUCT'">
          <h3>企业形态</h3>
          <div id="formBox"></div>
        </div>
        <div class="way-div" :class="{'way_div_pdt':subType=='COSMETIC_PRODUCT'}">
          <h3>风险等级</h3>
          <div id="riskBox"></div>
        </div>
        <div class="way-div" :class="{'way_div_pdt':subType=='COSMETIC_PRODUCT'}">
...
<script>
  import {  getDept, } from 'api/statistics/statistics';
  ...
  export default {
    ...
    async mounted() {
      ...
      if(this.subType!='COSMETIC_PRODUCT'){
         await API.listDictUserItemByCodeAll(this.subTypeList[this.subType].dict).then(r => {
    ...
    methods: {
      ...
      getBasic() {
        API.getBasic(this.subType).then(r => {
          if(r.status){
            this.dataLis = r.data;
          }
        })
      ...
      getParticular() {
        ...
        API.getParticular(params,this.subType).then(r => {
      ...
      initThread() {
        ...
        let params = {
          ...
          startYear: this.threndDate.start,
          endYear: this.threndDate.end,
          type: this.threndDate.mode == 0 ? 'INCREMEMENT' : 'TOTALA',
          areaCodes: this.threndDate.areaCodes,
        }
        API.getTotalTrend(params,this.subType).then(r => {
      ...
      return {
         subTypeList:{
          COSMETIC_MANAGER:{
            name:'经营主体',
            dict:'COSM_MB_SUB',
            editorsPath:'/cosmeticswork/editorsManage/editors',
            ArchivesPath:'/cosmetics/manager/archives'
          } ,
          COSMETIC_PRODUCT: {
            name:'生产主体',
            dict:'COSM_PB_SUB',
            editorsPath:'/cosmeticswork/editorsProduction/editors',
            ArchivesPath:'/cosmetics/product/archives'
          } ,
          COSMETIC_EMPLOY:{
            name:'使用主体',
            dict:'COSM_EB_SUB',
            editorsPath:'/cosmeticswork/editorsUse/editors',
            ArchivesPath:'/cosmetics/employ/archives'
          }
        },
        dataLis: {
