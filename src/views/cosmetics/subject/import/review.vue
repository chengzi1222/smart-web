<template>
  <div class="user-box">
    <div class="ipt-btn-box clear_a">
      <el-input :placeholder="searchMap.entityType=='COSMETIC_PRODUCT'?'许可证编号、主体名称、社会信用代码':'主体名称、社会信用代码'" class="w250 seekIpt" v-model="searchMap.keysWork" @keyup.enter.native="">
        <i slot="suffix" class="el-input__icon el-icon-search" @click="pageSearch"></i>
      </el-input>
      <enum-select class="selectHeight" v-if="searchMap.entityType=='COSMETIC_PRODUCT'" multiple v-model="searchMap.licState" placeholder="许可证状态" enum="com.ybveg.govx.enums.LicenceStateEnum" @change="pageSearch"></enum-select>
    ...
    <div class="tabOffTop120">
      <el-table @sort-change="sortChange"  :border="true" :resizable="true"  :data="tableData" @selection-change="selectChange" class="table-div">
        ...
        <el-table-column show-overflow-tooltip prop="backupNo" min-width="120"   v-if="searchMap.entityType=='COSMETIC_PRODUCT'"  label="*许可证编号"></el-table-column>
        <el-table-column show-overflow-tooltip prop="backupState" min-width="120"  v-if="searchMap.entityType=='COSMETIC_PRODUCT'"  label="*许可证状态"></el-table-column>
...
<script>
  ...
  import { page, review, del} from 'api/cosmetics/importentity'
