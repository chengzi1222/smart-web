  ...
  <div class="user-box">
    ...
    <transition name="dialog-fade">
      <popup-body class="popup-body" v-if="rejectIsShow">
        ...
        <button type="button" v-if="type === 'CHANGE'" @click="rejectEdit" class="w100 el-button el-button--primary" style="float: right;margin-left: 20px;">
          <span type="primary">确定</span>
        </button>
    ...
    <div class="main-body">
      ...
      <detail-component :workInfoTable='workInfoTable' :data="workInfo" :work-id="workId" v-else-if="tab === 'detail'"></detail-component>
...
export default {
  components: {
    ...
    //  WorkflowRecord
    BackupEditRecord
    ,ChangeDetail, DetailComponent
    , PunishRecord, PatrolRecord
  },
  created() {
  ...
  methods: {
    ...
    rejectEdit(){
      ...
      plan.postReviewEdit(param).then((result) => {
        if (!result.status) {
          this.$message({
            message: result.message,
            type: result.level
