  ...
  <div class="user-box">
    <div class="ipt-btn-box" style="justify-content: end;">
      ...
      <enum-select class="selectHeight" v-model="condition.workTypes" multiple collapse-tags placeholder="业务类型" @change="searchTab"
        enum="com.ybveg.govx.enums.sub.SubWorkTypeEnum">
      </enum-select>
      ...
      <el-table :data="tableData" :border="true" :resizable="true" @sort-change="sortChange" class="table-div" v-loading="loading">
        ...
        <el-table-column label="操作" min-width="150">
          <template slot-scope="scope">
            <a v-if="scope.row.type != 'EDIT'" href="javascript:;" @click="viewClick(scope.row)" class="operate-a">详情</a>
            <a v-if="scope.row.type == 'EDIT'" href="javascript:;" @click="viewClickEdit(scope.row)" class="operate-a">详情</a>
            <a v-if="scope.row.state == 'ALREADY_REJECT'" href="javascript:;" @click="reapplyEdit(scope.row)" class="operate-a">重新申请</a>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      reapplyEdit(row) {
        this.$router.push({
          path:'/cosmetics/product/archives/detail',
          query: {
