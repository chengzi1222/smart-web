<template>
  <div class="user-box">
    ...
    <div class="ipt-btn-box" style="justify-content:flex-end">
      ...
      <div class="floatR">
        <el-button class="selectHeight" v-auth="'medi.patrol.point|func.config'" type="primary" @click="showAdd">添加巡查要点表
        </el-button>
      </div>
    ...
    <div class="tabOffTop120">
      <el-table :resizable="true" :data="tableData" class="table-div" v-loading="loading">
        ...
        <el-table-column show-overflow-tooltip min-width="50" prop="projectNumber" label="巡查项目个数"></el-table-column>
        <el-table-column show-overflow-tooltip min-width="50" prop="contentNumber" label="巡查内容个数"></el-table-column>
        ...
        <el-table-column label="操作" v-if="canOperate()" min-width="120">
          <template slot-scope="scope">
            <a href="javascript:;" @click="goDetail(scope.row);" class="operate-a">查看</a>
            <a href="javascript:;" @click="goContext(scope.row);" v-auth="'medi.patrol.point|func.config'" class="operate-a">配置</a>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      /* link 方法 */
      goContext(table) {
        this.$router.push({ name: 'gistForm.contextList', query: { patrolTableId: table.id, name: table.name } });
      },

      goDetail(table) {
        this.$router.push({ name: 'gistForm.DetailedList', query: { patrolTableId: table.id } });
      },
