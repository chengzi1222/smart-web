  ...
  export default {
    ...
    methods: {
      ...
      goProject() {
        ...
        this.$router.push({name: "gistForm.projectList", query: {"patrolTableId": this.patrolTableId, 'name': name}})
      },

      goBack() {
        this.$router.push({name: 'gistForm.gistFormIndex'});
      },
    ...
    data() {
      return {
        ...
        editFn: (index, subIndex) => {
          ...
          if (this.tableData[index].children[subIndex].important == '是') {
            this.important = true;
          } else if (this.tableData[index].children[subIndex].important == '否') {
            this.important = false;
          }
        },
        //点击状态切换的按钮
