            ...
            <div class="content">
              ...
              <div class="row">
                <el-form-item label="许可备案号:">
                  <span> {{baseInfo.backupNo ? baseInfo.backupNo  : '暂无'}}</span>
                </el-form-item>
                <el-form-item label="复查记录编号:">
                  <span> {{baseInfo.patrolRecordNo ? baseInfo.patrolRecordNo  : '暂无'}}</span>
                </el-form-item>
              </div>
              ...
              <div class="row">
                ...
                <el-form-item label="巡查结果:">
                  <span>{{baseInfo.sourceResult ? baseInfo.sourceResult  : '暂无'}}</span>
                </el-form-item>
              </div>
              ...
              <el-table v-for="(item,index) in tableList" :key="index"
                ...
                <el-table-column label="是否检查通过" min-width="80">
                  <template slot-scope="scope">
                    {{scope.row.isSuit==='YES'?'检查通过':'检查不通过'}}
                  </template>
                </el-table-column>
            ...
            <div class="content">
              ...
              <div class="row">
                <el-form-item label="上传图片：" prop="">
                  <div class="imgbox">
                    <div v-for="(item,index) in resultInfo.imgItem" @click.stop="ifbig(item)"
                         class="imgdiv">
...
<script>
  ...
  export default {
    ...
    async mounted() {
      ...
      this.isAwait = false;
      //基础信息请求参数
      await api.getBascInfo({'id': this.repatrolRecordId}).then(d => {
        if (d.status && d.data) {
          this.baseInfo = d.data[0];
          this.areaCode = d.data[0].patrolDept;
    ...
    methods: {
      back() {
        if (this.$route.query.isList == 'YES') {
          this.$router.push({name: 'cosmetics.RepatrolRecordList', query: {'type': this.messType}});
        } else {
          this.$router.go(-1)
        }
      },
      ...
      async printInitData(id, type) {
        ...
        await api.getBascInfo({'id': id}).then(d => {
          if (d.status && d.data) {
            ...
            this.basicInfoDataPrint.licenseNumber = baseInfo.backupNo;
            this.basicInfoDataPrint.spaceAddr = baseInfo.realAddress;
            this.basicInfoDataPrint.corpName = baseInfo.legalPerson;
            this.basicInfoDataPrint.mobile = baseInfo.corpMobile;
            ...
            this.resultDataPrint.recordNo = baseInfo.patrolRecordNo
        ...
        await api.getResultInfo(objResult).then(d => {
          if (d.status && d.data) {
            ...
            this.resultDataPrint.treat = resultInfo.treatStr;
