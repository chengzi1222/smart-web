  ...
  <div v-show="printShow" class="user-box" id="content">
    <div class="printContent">
      ...
      <div id="printBody">
        ...
        <hr>
        <!-- 检查结果 -->
        <p class="text_indent" v-if="resultData.treat === '再次限期整改'"> 针对上述问题，巡查结果为：<u>不符合</u>；处理方式为：<u>限期整改</u>。责令经营者务于
        ...
        <p class="text_indent" v-if="resultData.treat === '调查处理'">
          针对上述问题，巡查结果为：<u>不符合</u>；处理方式为：<u>调查处理</u>。 </p>
        <p class="text_indent" v-if="resultData.treat === '复查通过'">针对上述问题，巡查结果为：<u>符合</u>；处理方式为：<u>正常，无需处理</u>。
        </p>
        <!-- 检查结果结束-->
