...
<template>
  <div v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(255, 255, 255, 0.8)"
        ...
        <el-form :model="ruleForm" ref="ruleForm" label-width="110px" class="demo-ruleForm">
          ...
          <toggle-form title="基本信息">
            <div class="content" v-if="basicInfoData">
              ...
              <div class="row">
                ...
                <el-form-item :label="infoTitle" prop="" v-if="$route.query.patrolAreaCode.length > 6">
          ...
          <toggle-form title="巡查项（仅显示本次检查的巡查项）">
            <div class="mT35" v-if="patrolData">
              <div>
                <span style="vertical-align: top;margin-top: 7px;display: inline-block;margin-right: -20px;">查看模式：</span>
                <el-select @change="getPatrolContext" class="w200 selectHeight" v-model="contextType">
          ...
          <toggle-form title="巡查结果">
            <div class="content" v-if="resultData">
              ...
              <div class="row ml30" style="margin: 20px 0">
                <span style="font-size: 18px;color: #393939;">企业得分：</span><span style="color: #0DB5EF;font-size: 18px; "><span>
                  {{resultData.qualified}}</span>分</span>
                <span style="font-size: 12px;color: #A2AAB6;margin-left: 40px;">（满分：{{basicInfoData.score}}分，此评分仅供参考）</span>
              </div>
              <div class="row">
              ...
              <div class="row">
                <el-form-item label="上传图片：" prop="">
                  <div class="imgbox" v-if="basicInfoData.upfileList.length >= 1">
                    <div v-for="(item,index) in basicInfoData.upfileList" @click.stop="ifbig(item.fileId)" class="imgdiv">
              ...
              <div class="row mRno">
                <el-form-item label="企业电子签名：" prop="">
                  ...
                  <div v-if="basicInfoData.isCooperate == 'YES'" @click.stop="ifbigq()" class="imgdiv" :class="ifbigqy?'imgbig':''">
    ...
    <!-- 打印 -->
    <PrintPatrol ref="PrintPatrol" :basicInfoData="basicInfoDataPrint" :messengers="messengersPrint" :patrolData="patrolDataPrint"
  ...
  export default {
    ...
    methods: {
      ...
      initData() {
        ...
        planApi.getPatrolRecord(this.recordId).then(d => {
      ...
      getPatrolContext(){
        ...
        planApi.getPatrolContext(this.recordId, params).then(d => {
          this.patrolData = d.data.reList;
        })
      },
      goEntityDatail() {
        const entityId = this.basicInfoData.entityId;
        let strType = this.basicInfoData.entityType.substring(this.basicInfoData.entityType.indexOf("_") + 1)
        this.$router.push({
