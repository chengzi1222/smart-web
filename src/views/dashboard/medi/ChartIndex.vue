    ...
    <div class="main-body" style="padding:0 30px 0 50px;box-sizing: border-box;">
      <section class="charts">
        ...
        <section style="width:48.5%" class="charts-item">
          ...
          <div class="floatL charts-item-left" :class="{'canHover':entityShow}"  @click="$router.push(entityShow ? '/mediSub/subMap': '')">
        ...
        <section style="width:48.5%" class="charts-item clearfix">
          ...
          <div class="floatL charts-item-left"  :class="{'canHover':entityShow}"  @click="$router.push(entityShow ? '/mediSub/subMap?index='+entityTypePei_mapIndex: '')">
        ...
        <section style="width:62%;">
          <section style="width:100%;position:relative" class="charts-item">
            ...
            <div class="floatL charts-item-left" style="width:29.5%;margin: 8% 0%" :class="{'canHover':dailyShow}"
              @click="$router.push(dailyShow ? '/medipatrol/record/daily/list?startTime='+getdate(dailyStartTime)+'&endTime='+getdate(dailyEndTime): '')">
        ...
        <section style="width:37%" class="charts-item">
          <div class="clearfix">
            <p class="title" >药品预警</p>
            <div class="floatR">
              <el-date-picker @change="mediWarning" class="w140" v-model="mediWarningData.startTime" type="date" placeholder="起始时间" :picker-options="pickerOptions7">
              </el-date-picker>
              <span>至</span>
              <el-date-picker @change="mediWarning" class="w140" v-model="mediWarningData.endTime" type="date" placeholder="截止时间" :picker-options="pickerOptions8">
              </el-date-picker>
            </div>
            <div style="clear:both"></div>
            <div class="kitchen mt20" >
              <div class="kitchen-item" :class="{'canHover':cameraShow}"  @click="$router.push(cameraShow ? '/medi/warning/list?index=1': '')">
                <i class="icon icon-style iconfont icon-wancheng-copy"></i>
                <p class="num">{{mediWarningData.alreadyCompleted}}
                  <span class="unit">(家)</span>
                </p>
                <p>已完成</p>
              </div>
              <div class="kitchen-item" :class="{'canHover':cameraShow}"  @click="$router.push(cameraShow ? '/medi/warning/list?index=0': '')">
                <i class="icon icon-style iconfont icon-zhixingzhong"></i>
                <p class="num">{{mediWarningData.executing}}
                  <span class="unit">(家)</span>
                </p>
                <p>执行中</p>
              </div>
...
<script>
  ...
  import {
    ...
    getEntityAll,
    getEntityType,
  } from 'api/medi/mediSub/mediIndex'
  ...
  export default {
    ...
    mounted() {
      ...
      this.mediWarningData.endTime = dates.getFullYear() + "-" + mon + "-" + day;
      ...
      this.mediWarningData.startTime = dates.getFullYear() + "-01-01";

      this.listDictUserItemByDictCode('MEDI_QUANTIFY');
      this.listDictUserItemByDictCode('MEDI_RISK');
      this.getEnums(['com.ybveg.govx.enums.medi.MediEntityTypeEnum']);
      ...
      //主体数量统计
       this.statsEntityType('MEDICINE_MANAGER')
       this.statsEntityAll()
    ...
    data() {
      return {
        ...
        entityTypePei:'',
        entityTypePei_mapIndex:'MEDICINE_MANAGER',//跳转主体地图的type
        EntityTypeData: {
          name:[],
          data:[],
          total:0
        ...
        pickerOptions6: {
          ...
          disabledDate: (time) => {
            let endDateVal = new Date(this.mediWarningData.endTime);
            if (this.mediWarningData.endTime) {
                return time.getTime() > endDateVal;
            }
          }
        },
        pickerOptions8: {
          disabledDate: (time) => {
            let beginDateVal = new Date(this.mediWarningData.startTime);
            if (this.mediWarningData.startTime) {
              return time.getTime() < beginDateVal ;
            }
          }
        },
    ...
    methods: {
      ...
      dailyFn() {
          let obj={
            startDate:this.getdate(this.dailyStartTime).replace(new RegExp(/(\-)/g),'/'),
            endDate:this.getdate(this.dailyEndTime).replace(new RegExp(/(\-)/g),'/'),
            type:'DAILY',
          }
        getPatrolPlan(obj).then(res => {
          ...
          this.dailyShow = hasAuth("medi.patrol.record");
      ...
      specialFn() {
         let obj={
          startDate:this.getdate(this.startTime).replace(new RegExp(/(\-)/g),'/'),
          endDate:this.getdate(this.endTime).replace(new RegExp(/(\-)/g),'/'),
          type:'SPECIAL',
        }
        getPatrolPlan(obj).then(res => {
          ...
          if (hasAuth("medi.special.plan")) {
            this.special[0].path = "/medipatrol/special/list?index=2";
            this.special[1].path = "/medipatrol/special/list?index=0";
          }
           if (hasAuth("medi.patrol.record")) {
            this.special[2].path = "/medipatrol/record/special/list";
          }
        })
      },
      //动态巡查
      ...
      mediWarning() {
        let obj={
          startDate:this.getdate(this.mediWarningData.startTime).replace(new RegExp(/(\-)/g),'/'),
          endDate:this.getdate(this.mediWarningData.endTime).replace(new RegExp(/(\-)/g),'/')
        }
        getMediWarning(obj).then(res => {
          if(res.status&&res.data){
            this.mediWarningData.executing=res.data.executing;
            this.mediWarningData.alreadyCompleted=res.data.alreadyCompleted;
            this.cameraShow = hasAuth("medi.biz.wb");
          }
      ...
      homeEntityStatFn() {
        ...
        getWarningArea().then(res=>{
          ...
          res.data.riskRankWarning.forEach(a=>{
            ...
            this.expire[0].num = res.data.currentTask.mediWarning?res.data.currentTask.mediWarning:0;
            this.expire[1].num = res.data.currentTask.specialTask?res.data.currentTask.specialTask:0;
        ...
        getHomeEntityStat().then(res => {
          ...
          if (hasAuth("medi.biz.mb|func.review")) {
            this.backlog[0].path = '/medi/workflow/medicine_manager/list';
          }
        })
      },
      entityTypeFun(){
        if(this.entityTypePei==''){
          this.entityTypePei_mapIndex='MEDICINE_MANAGER';
        }else{
      ...
      statsEntityAll(){
        getEntityAll().then(res => {
          ...
          this.entityTotal = count.a+count.b;
          this.entityTotalMap = res.data;
          this.entityShow = hasAuth("medi.subject.map");
      ...
      listDictUserItemByDictCode(dictCode) {//字典
        plan.listDictUserItemByDictCode(dictCode).then((data) => {
          for (let i = 0; i < data.data.length; i++) {
            if (data.data[i].code === 'NOASSESS') {
              data.data.splice(i, 1)
            }
          }
          if (dictCode == 'MEDI_RISK') {
