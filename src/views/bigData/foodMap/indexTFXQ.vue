<template>
  <baidu-map class="bm-view" :theme='arr' :ak="baiduMapAk" @moveend="zoomend" :center="centerComputed" :zoom="zoomComputed" :min-zoom="zoomComputed-2" :map-click="false" :max-zoom="map.maxZoom" :scroll-wheel-zoom="map.scrollWheelZoom" @load="load" @zoomend="zoomend" @ready="handler">
    <EntityInfoOverlayTFXQ :position="entityInfo.position" :active="entityInfo.active" :info="entityInfo.data" @mouseover.native="active = true" @mouseleave.native="active = false">
    </EntityInfoOverlayTFXQ>

    <ChartsBoxTFXQ></ChartsBoxTFXQ>

    <SearchBoxTFXQ @show="searchShow"></SearchBoxTFXQ>

    <AreaOverlayTFXQ  v-for="item in overlays" :key="item.areaCode" :area-name="item.areaName" :num="item.num" :lng="item.lng" :lat="item.lat"></AreaOverlayTFXQ>

    <div v-if='is'>
      <EntityPointTFXQ v-for="item in points" :key="item.id" :active-id="activeId" @actived="entityActived" :point="item"></EntityPointTFXQ>
    </div>
...
import ChartsBoxTFXQ from "./mapTFXQ/ChartsBox";
import SearchBoxTFXQ from './mapTFXQ/SearchBox';
import AreaOverlayTFXQ from './mapTFXQ/AreaOverlay';
import EntityInfoOverlayTFXQ from './mapTFXQ/EntityInfoOverlay';
import EntityPointTFXQ from './mapTFXQ/EntityPoint';
...
export default {
  ...
  computed: {
    ...
    zoomComputed() {
      return this.$store.getters.mapCenter.zoom + 1;
    }
  },
  components: {
    BaiduMap, BmView,ChartsBoxTFXQ,SearchBoxTFXQ,EntityInfoOverlayTFXQ, AreaOverlayTFXQ, EntityPointTFXQ
  }
};
</script>
