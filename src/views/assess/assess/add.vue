<template>
	<div>
		<div class="page-tilt mb20">
			<b></b><span>考核计划{{type=='add'?'添加':'编辑'}}</span>
		</div>
		<div style="margin-bottom: 120px;position: relative;" >
			...
			<div class="ml210">
				...
				<Questions v-if='show' :id='id'></Questions>
			...
			<div class="foot-btn" style="z-index:980" v-else>
				<div>
					<el-button class="w100 mr10" @click="$router.push('/assess/index/list')">返回</el-button>
					<el-button class="w100 mr10" v-if="status=='1'" type="primary" @click="startTrainPlan">启动并返回
					</el-button>
				</div>
			</div>
		</div>
...
<script>
	...
	import Questions from "./com/questions.vue";
	...
	export default {
		...
		methods: {
			...
			back() {
				...
				this.$confirm("正在返回考核列表，你当前的操作将默认待启动状态！", '提示', {
					confirmButtonText: '确定',
					type: 'success'
				}).then(() => {
					setTimeout(() => {
						this.$router.push('/assess/index/list')
					}, 1500)
				})
			},
			startTrainPlan() {
