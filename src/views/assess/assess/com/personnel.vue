<template>
  <div>
    <ToggleForm title="考核人员管理">
      <div class="mt30">
        <div class="mb20">
          ...
          <el-button class="w120" type="primary" @click="downTableTemplet()">下载导入模板</el-button>
        ...
        <el-table v-loading="loading" @selection-change="handleSelectionChange" element-loading-text="数据正在努力加载中"
          ...
          <el-table-column show-overflow-tooltip min-width="100" label="考核结果" v-if='type=="edit"'>
            <template slot-scope="scope">
    ...
    <!-- 弹窗 -->
    <yb-popup @close="close" :isShow="isShow" title="添加考核人员" width="1030">
      <div slot="body">
        <div style="margin:0 20px 20px">
          ...
          <el-select class="mr20 w150" v-model="personType" @change="changeSearchPop" :clearable="true" multiple
  ...
  export default {
    ...
    methods: {
      ...
      uploadConfig(file, callback) {
        ...
        api.uploadFile(formdata,1).then((r) => {
      ...
      searchPagePop() {
        ...
        this.queryLimitPop.personType = this.personType.toString()
        api.assessPlanPersonInnerList(this.queryLimitPop).then(result => {
          if (result.status) {
      ...
      submit() {
        ...
        api.assessPlanPersonUpdate({
          planId: this.id,
          personList: this.selList
        }).then(r => {
          if (r.status) {
