			...
			<div class="mt30 ">
				<div class="mb20 clear_a">
					...
					<el-button class="w120 fr ml20" type="primary" @click="add('add',null)" v-if='num<=2000'>
						<i style="margin-right: 5px;font-weight: 700;color: #fff;" class="el-icon-plus"></i>添加判断题
					</el-button>
				...
				<el-table v-loading="loading" element-loading-text="数据正在努力加载中" :border="true" :resizable="true"
					...
					<el-table-column show-overflow-tooltip min-width="300" label="题目" prop="title"></el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			//下载要点表导入模板
			downTableTemplet() {
				location.href = `/api/pxkh/question/template?type=judge`;
			},
			...
			uploadConfig(file, callback) {
				...
				api.uploadFileQuestion2(formdata).then((r) => {
			...
			add(type, row) {
				...
				this.addPopTypeTitle = type == 'add' ? '添加判断题' : '编辑判断题';
				this.isShow = true;
				this.addData = {
					id: '',
			...
			del(row) {
				this.$confirm("是否确定删除该题?", '提示', {
					...
					api.questionJudgeDel(data).then(r => {
						if (r.status) {
			...
			searchPage() {
				...
				api.questionJudgeList(this.queryLimit).then(result => {
					if (result.status) {
			...
			addPop(again) {
				if (this.addData.title == '' || this.addData.correct == '' || this.addData.resolve == '') {
				...
				data.append('correct', this.addData.correct * 1)
				data.append('resolve', this.addData.resolve)
				api.questionJudgeAddOrUpdate(data, this.addPopType).then((r) => {
					if (!r.status) {
						this.$message({
							message: r.message,
							type: 'error'
