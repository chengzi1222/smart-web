<template>
    <div id="subjectPie">
       ...
       <div class="clear_a btnBox">
           <div class="floatL" :class="{'active':active==='subType'}" @click="active='subType'" style="cursor: pointer;">主体总数</div>
           <div class="floatR" :class="{'active':active==='risk'}"  @click="active='risk'" style="cursor: pointer;">风险等级分布</div>
       </div>
       <div class="echartBox">
            <div class="echart" id="subType" :class="active=='subType'?'echartIndex':''" >主体总数</div>
...
<script>
    ...
    export default {
        ...
        methods: {
            getSubSurveyData() {
                subjectProfile.entityDistributionDemo().then(r => {
                    if(r.status === true) {
                        this.subTypeData = r.data;
                        echartFun.echartsDoughnut(this.subTypeData, 'subType');
                    }
                }).catch(e=>{})
                subjectProfile.riskLevelDtrDemo().then(r => {
                    if(r.status === true) {
