<template>
    <baidu-map id="map" :mapStyle='arr' :ak="baiduMapAk" :center="centerComputed" :zoom="zoomComputed"
        :min-zoom="zoomComputed-2" @moveend="zoomend" :map-click="false" :max-zoom="map.maxZoom"
        :scroll-wheel-zoom="map.scrollWheelZoom" @ready="handler" @load="load" @zoomend="zoomend" @dragend="dragend">
        <ul class="operate absolute clear_a">
            <li :disabled="btn" @click="select('high')"><span class="circle"
                    :class="{ 'redC':highSelect === true}"></span><span>高风险</span></li>
            <li :disabled="btn" @click="select('middle')"><span class="circle"
                    :class="{'yellowC':middleSelect === true}"></span><span>中风险</span></li>
            <li :disabled="btn" @click="select('low')"><span class="circle"
                    :class="{ 'blueC': lowSelect === true}"></span><span>低风险</span></li>
        ...
        <select-point v-for="item in points" :key="item.entityId" :zoom="zoomNum" :active-item="activeItem"
            @actived="entityActived(item)" @close="closePoint" :point="item"></select-point>
...
<script>
    ...
    import * as mapApi2 from 'api/xcws/warnStatistics/map'
    import {
        getTheme
    } from "@/views/xcws/warnStatistics/getTheme";
    ...
    export default {
        ...
        async mounted() {
            if (!document.createElement('canvas').getContext) {
                this.$message({
                    type: 'warning',
                    message: '使用chrome、safari、IE9+以上浏览器才能正常访问此页面'
                });
            }
        ...
        methods: {
            ...
            async comfirmType(code) {
                ...
                mapApi2.pageDemo(this.queryLimit).then(d => {
                    if (d.status) {
                        this.cache = d.data;
                        this.points = d.data;
                    }
        ...
        computed: {
            centerComputed() {
                ...
                return {lng: this.$store.getters.mapCenter.lng, lat:this.$store.getters.mapCenter.lat}
            },
            zoomComputed() {
