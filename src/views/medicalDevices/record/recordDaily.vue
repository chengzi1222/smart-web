<template>
  <div v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(255, 255, 255, 0.8)"
    ...
    <div class="ipt-btn-box">
      ...
      <enum-select v-model="search.entityType" multiple placeholder="主体类型" @change="handleIconClick" enum="com.ybveg.govx.enums.maai.MaaiEntityTypeEnum">
      </enum-select>
      <!-- <i :class="down?'el-icon-caret-top':'el-icon-caret-bottom'" class="down" @click="downSeleBox"></i> -->
      <el-button class="selectHeight floatR" type="primary"  @click="add" v-auth="'maai.patrol.record|func.add'">
      ...
      <el-table :data="tableData" class="table-div" :border="true" :resizable="true" :class="shadowShow?'shadowNo':''" @sort-change="handleSortChange"
        ...
        <el-table-column label="操作" fixed="right" min-width="150">
          <template slot-scope="scope">
            <a href="javascript:;" @click="view(scope.row.recordId, scope.row.patrolAreaCode)" class="operate-a">详情</a>
            <a href="javascript:;" @click="printRecord(scope.row.recordId, true)" class="operate-a">打印预览</a>
            <a href="javascript:;" @click="printRecord(scope.row.recordId, false)" class="operate-a">打印</a>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 打印组件 -->
    <PrintPatrol ref="PrintPatrol" :basicInfoData="basicInfoData" :patrolContent="patrolContent"
      :resultData="resultData" :patrolType="'日常监督抽查'" :qrcodeId="qrcodeId">
    </PrintPatrol>
...
<script type="module">
  ...
  import {postRecordPage,getPatrolRecord} from "api/maai/record/recordDaily";
    ...
    methods: {
      ...
      async getPage() {
        var formData = {
          ...
          patrolPlanType: 'DAILY'
        }
        this.loading = true;
        postRecordPage(this.page.num, this.page.pageSize,formData).then(r => {
          this.loading = false;
      ...
      async printInitData(id, type) {
        this.qrcodeId = id;
        await getPatrolRecord(id, type).then(d => {
          this.resultData = d.data.basic;
          this.resultData.planName = d.data.basic.entityTypeStr + "日常监督检查";
