            ...
            <div class="content">
              <div class="row"  v-if="$route.query.type == 'special'">
                <el-form-item label="巡查计划:" prop="" style="margin-bottom:0px;">
                  <select-plan @slplan="slplan" :currentIndex='currentIndex2' enumClass="com.ybveg.govx.enums.maai.MaaiEntityTypeEnum" :apiFn='selectPlanApiFn'></select-plan>
                </el-form-item>
              </div>
              ...
              <div class="row" v-if="ruleForm.planName || $route.query.type == 'daily'">
                <el-form-item label="主体信息:" prop="">
                  <select-mainbody @slmainbody="slmainbody" @citem="citem" :currentIndex='currentIndex' :apiFn='selectMainbodyApiFn' :entryType='$route.query.type'
                    :enumClass='"com.ybveg.govx.enums.maai.MaaiEntityTypeEnum"' :taskGroupId="ruleForm.patrolPlanId" :bizType = "ruleForm.bizType" :key="ruleForm.patrolPlanId" :changeItem="changeItem"></select-mainbody>
                </el-form-item>
              </div>
              <!-- 选择的主体 -->
              <div class="mainbody" v-if='~currentIndex'>
                ...
                <div class="row">
                  <div class="form-item">
                    <div class="title">器械类别：</div>
                    <span>{{ruleForm.categoryStr}}</span>
                  </div>
                  <div class="form-item">
              ...
              <div class="row">
                <el-form-item :label="$store.getters.user.areaCode.length > 6?'监管人员':'检查人员'" prop="patrolSoId">
                  <el-select @change="blurCount" class="w200" v-model="ruleForm.patrolSoId" multiple collapse-tags :placeholder="$store.getters.user.areaCode.length > 6?'请选择监管人员':'请选择检查人员'" :multiple-limit='5'>
                    <el-option v-for="item in ruleForm.soList" :key="item.userId" :label="item.realName" :value="item.userId"  v-auth="'maai.patrol.record|func.add'"></el-option>
                  </el-select>
                </el-form-item>
              </div>
              ...
              <div class="row" v-if="$store.getters.user.areaCode.length > 6">
                <el-form-item :label="infoTitle" prop="">
                  <el-select @change="blurCount" class="w200" v-model="ruleForm.infoList" multiple collapse-tags :placeholder="infoPlaceholder" :multiple-limit='5'>
                    <el-option v-for="item in ruleForm.info" :key="item.userId" :label="item.realName" :value="item.userId"  v-auth="'maai.patrol.record|func.add'"></el-option>
                  </el-select>
                </el-form-item>
              </div>
          ...
          <toggle-form title="巡查项">
            <div v-if="patrolData.length>0" style="margin-top:35px;">
              <el-table :key="index" v-for="(item,index) in patrolData" :class="index==0?'firsTeit':''" :data="item.conList" :span-method="objectSpanMethod"
                ...
                <el-table-column show-overflow-tooltip prop="item" label="巡查项目" min-width="120">
                  <template slot-scope="scope">
                    {{patrolData[index].name}}
                  </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip label="巡查内容编号" min-width="120">
                  <template slot-scope="scope">
                    <span>{{patrolData[index].pno}}.{{patrolData[index].conList[scope.$index].cno}}</span>
                  </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip prop="context" label="巡查内容" min-width="300"></el-table-column>
                ...
                <el-table-column show-overflow-tooltip prop="qualStr" label="是否检查通过" min-width="180">
                  <template slot-scope="scope">
                    <single-checkbox :currentRowData='scope.row' @singlecheckboxchange="singlecheckboxchange"></single-checkbox>
                  </template>
                </el-table-column>
                <el-table-column show-overflow-tooltip prop="remark" label="备注" min-width='130'>
                  <template slot-scope="scope">
                ...
                <el-table-column align="center" label="附件" min-width='80'>
                  <template slot-scope="scope">
                    <el-upload action="/api/upload" :before-upload="beforeUpPic" :on-success="picSuccess" :data="{type:'MAAI_PATROL',field:'PHOTO'}"
                ...
                <el-table-column label="附件" min-width="160">
                  <template slot-scope="scope" v-if="scope.row.isShowFile">
                    <div class="accessoryList">
                      <span class="movePoin" @mouseenter="accessoryList($event,scope.row)" @mouseleave="accessoryListElse">
                        <i class="el-icon-picture-outline"></i>&nbsp;附件列表
                      </span>
                    </div>
                  </template>
                </el-table-column>
          ...
          <toggle-form title="巡查结果">
            <div class="content">
              ...
              <div class="row ml30" style="margin: 20px 0">
                <span style="font-size: 18px;color: #393939;">企业得分：</span><span style="color: #0DB5EF;font-size: 18px; ">{{ruleForm.patrolScore}}分</span>
                <span style="font-size: 12px;color: #A2AAB6;margin-left: 40px;">（满分：{{ruleForm.score}}分，此评分仅供参考）</span>
              </div>
              <div class="row mRno">
              ...
              <div class="row">
                <el-form-item label="上传图片：" prop="">
                  <el-upload :file-list="photo" :class="picList.length <=9 ? '':'upload-hide'" action="/api/upload" list-type="picture-card" :on-success="uploadSuccess" :on-error="uploadError"
                    :data="{type:'MAAI_PATROL',field:'PHOTO'}" :before-upload="beforeAvatarUpload" :on-preview="handlePictureCardPreview"
                    :on-remove="removePic">
              ...
              <div class="row">
                <el-form-item label="风险分级：" prop="">
                  <dict-select @change="blurCount" v-model="ruleForm.quanLevel" dict="MEDI_QUANTIFY" placeholder="选择风险分级"></dict-select>
                  <span style="font-size: 12px;color: #A2AAB6;">(选填)</span>
                </el-form-item>
              </div>
              ...
              <div class="row mRno">
                ...
                <el-upload :file-list="sign" :class="picListT.length==0 ? '':'upload-hide'" v-if="ruleForm.isCooperate == 'YES'" action="/api/upload" list-type="picture-card" :on-success="uploadSuccessT"
                  :on-error="uploadError" :data="{type:'MAAI_PATROL',field:'SIGN'}" :before-upload="beforeAvatarUpload"
...
<script type="module">
  ...
  import * as planApi from "api/maai/record/recordSpecial";
  ...
  export default {
    ...
    methods: {
      ...
      tableChange() {
        ...
        for (let obj of data) {
          ...
          for (let subObj of subData) {
            this.ruleForm.score += subObj.score * 1;
        ...
        this.ruleForm.patrolScore = this.ruleForm.score - deduct;
      },
      //表格选择是否符合
      singlecheckboxchange(obj) {
        let ci = this.checkImportant ;
        let { row, result } = obj;
        ...
        //选择了是否符合，标记该行有效,否则无效
        if (result) {
          if(row.important == "YES" && !row.tag){
            ci++;
          }
          row.tag = true;
          this.ifContNum.push(row.patrolContextId)
        }
        this.checkImportant = ci;
        //如果评论不存在，为该行评论添加观察者
        if (!row.remark) {
          this.$set(row, 'remark', '');
        }
          ...
          if (valid) {
            this.$confirm('是否要提交巡查记录?', '提示', {
              ...
              obj.recordPo = {
                ...
                tableId:this.ruleForm.tableId,
                // tableName:this.ruleForm.tableName,
                patrolPlanName:this.ruleForm.planName
              };
              obj.repatrolRecordPo = {
                entityId,
              ...
              console.log(this.minImportant);
              console.log(this.checkImportant);
              debugger
              if(this.checkImportant<this.minImportant){
                this.$message({
                  message: '最少巡查重点项不能低于'+this.minImportant+'个',
                  type: 'warning'
                })
                return false
              }
      ...
      slmainbody(obj) {
        ...
        this.ruleForm.creditCode = obj.creditCode;
        this.ruleForm.categoryStr = obj.categoryStr;
        ...
        //获取巡查项
        planApi.getContext(this.ruleForm.tableId).then(res => {
          const data = res.data;
          if (!data) {
