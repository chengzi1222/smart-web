<template>
  ...
  <div v-show="printShow" class="user-box" id="content">
    <div class="printContent" v-if="resultData.planName">
      <div id="printHead">
        <span id="qrcode"></span>
        <div class="printTitle" ref="title">现场检查笔录</div>
        <p class="number">编号：{{ resultData.patrolRecordNo }} <span id="printPage">第1页，共X页</span></p>
        <hr>
      </div>
      <div id="printBody">
        <p>检查事由： <u>{{ resultData.planName }}</u></p>
        <p>被检查单位（人）：<u> {{resultData.entityName}}</u></p>
        <p>统一社会信用代码： <u> {{resultData.creditCode}}</u></p>
        <p> 检查地点：<u> {{resultData.realityAddr}}</u></p>
        <p>
          <span style="width:350px; display: inline-block;"> 法人：<u> {{resultData.legalPerson}}</u></span>
          联系电话：<u> {{resultData.contactMobile}}</u>
        </p>
        <p><span style="width:450px; display: inline-block;"> {{resultData.deptAreaCode.length <= 6? '检查人：' : '监管人：'}}<u> {{resultData.checkNames}}</u></span>
          <span style="width: 450px;display: inline-block;" v-if='resultData.deptAreaCode.length > 6'>信息员：<u> {{resultData.accompanyNames}} </u></span>
        <p>检查类别： <u> {{patrolType}} </u></p>
        <p>检查时间： <u> {{resultData.patrolTime}}</u></p>
        <hr>
        <p class="text_indent">我们是<u> {{resultData.deptName}}</u>的执法人员 <u>{{resultData.checkNames}}、{{resultData.accompanyNames}}</u>，执法证件名称、编号是：
        ...
        <hr>
        <p class="text_indent" v-if="resultData.statistics"> 此次巡查共巡查重点项：<u> {{resultData.statistics.impItem}}</u>个；一般项： <u>
          {{resultData.statistics.simItem}}</u>个。
          其中重点项不符合：<u> {{resultData.statistics.impUnqua}}</u>个；
          一般项不符合：<u>{{resultData.statistics.simUnqua}}</u>个；
          符合的重点项的编号为：<u> {{resultData.statistics.impSuitNo?resultData.statistics.impSuitNo:'无'}}</u> 符合的一般项的编号为：<u>
            {{resultData.statistics.simSuitNo?resultData.statistics.simSuitNo:'无'}}</u></p>
        <p v-if="resultData.statistics.impList.length == 0" class="text_indent">重点项不符合项目： 无</p>
        <p v-if="resultData.statistics.impList.length > 0" class="text_indent">重点项不符合项目： </p>
        <p v-if="resultData.statistics.impList.length > 0" class="text_indent"
           v-for="(item, index) in resultData.statistics.impList"> {{index + 1}}，编号： {{item.pno + '.' + item.cno}}；内容：&nbsp;{{item.context}}&nbsp;备注：&nbsp;{{item.remark ? item.remark : '无'}}&nbsp; </p>
        <p v-if="resultData.statistics.simList.length == 0" class="text_indent"> 一般项不符合项目： 无</p>
        <p v-if="resultData.statistics.simList.length > 0" class="text_indent"> 一般项不符合项目： </p>
        <p v-if="resultData.statistics.simList.length > 0" class="text_indent"
           v-for="(item, index) in resultData.statistics.simList"> {{index + 1}}，编号： {{item.pno + '.' + item.cno}}；内容：&nbsp;{{item.context}}&nbsp;备注：&nbsp;{{item.remark ? item.remark : '无'}}&nbsp;</p>
        <hr>
        <p class="text_indent"> 针对上述问题，巡查结果为：<u>{{resultData.resultStr}}</u>；处理方式为：<u>{{resultData.treatStr}}</u>。<span v-if="resultData.treatStr == '限期整改'">责令经营者务于
          {{resultData.rectifyTime}} 前整改到位，并将整改情况反馈给 __________ （联系电话：__________ ）。</span> </p>
        ...
        <p v-if="!resultData.upfileList" class="text_indent">电子签名：<u>无</u></p>
        <p v-if="resultData.upfileList" class="text_indent">电子签名：<u><img :src="pathFile + resultData.upfileList" alt=""></u></p>
        <p class="text_indent">
