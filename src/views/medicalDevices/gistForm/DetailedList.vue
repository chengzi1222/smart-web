						...
						<div class="content toggle-form-c">
							<div class="row">
								...
								<el-form-item label="主体类型:" prop="">
									<span>{{patrolTable.entityTypeStr}}</span>
								</el-form-item>
							</div>
  ...
  import { listContext, getPatrolTable } from 'api/medi/patrol/project';
    ...
    mounted() {
      ...
      plan.postGet({id:this.patrolTableId}).then((result) => {
		...
		methods: {
      goContext() {
        this.$router.push({name: "medicalDevicesGistForm.contextList", query: { 'patrolTableId': this.patrolTableId,'name': this.patrolTable.name}})
      },
