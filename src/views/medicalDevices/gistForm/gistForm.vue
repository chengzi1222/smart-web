<template>
  <div class="user-box">
          ...
          <el-form :model="patrolTable" :rules="rules" ref="patrolTable" label-width="100px" class="demo-ruleForm">
            ...
            <el-form-item v-if="isAdd" label="主体类型:" prop="entityType">
              <el-select v-model="patrolTable.entityType" placeholder="请选择主体类型" class="w200">
                <el-option v-for="item in entityTypeList" :key="item.key" :label="item.value" :value="item.key">
                </el-option>
              </el-select>
            </el-form-item>
            <el-form-item v-if="isAdd" label="器械等级:" prop="entityType">
              <el-select v-model="patrolTable.instrumentType" placeholder="请选择器械等级" class="w200">
                <el-option v-for="item in instruranklist" :key="item.key" :label="item.value" :value="item.key">
                </el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="备注:" prop="name">
              <el-input type="textarea" placeholder="请输入备注,100字以内,选填" :maxlength="100" v-model="patrolTable.remark"
                        class="w200"></el-input>
            </el-form-item>
          </el-form>
    ...
    <div class="tabOffTop120">
      <el-table :resizable="true" :data="tableData" class="table-div" v-loading="loading">
        <el-table-column show-overflow-tooltip min-width="30" type="index" label="序号"></el-table-column>
        <el-table-column show-overflow-tooltip min-width="100" prop="name" label="名称"></el-table-column>
        <el-table-column show-overflow-tooltip min-width="50" prop="entityTypeStr" label="主体类型"></el-table-column>
        <el-table-column show-overflow-tooltip min-width="50" prop="instrumentTypeStr" label="器械等级"></el-table-column>
        <el-table-column show-overflow-tooltip min-width="50" prop="projectNum" label="巡查项目个数"></el-table-column>
        <el-table-column show-overflow-tooltip min-width="50" prop="contextNum" label="巡查内容个数"></el-table-column>
        <!-- <el-table-column show-overflow-tooltip prop="minImportant" label="最少重点内容"></el-table-column> -->
        <el-table-column show-overflow-tooltip min-width="50" prop="configTypeStr" label="类型"></el-table-column>
        <el-table-column show-overflow-tooltip min-width="40" prop="enableStr" label="状态"></el-table-column>
        <el-table-column show-overflow-tooltip min-width="40" prop="remark" label="备注"></el-table-column>
        <el-table-column show-overflow-tooltip min-width="80" prop="updateTime" label="更新时间"></el-table-column>

        <el-table-column label="操作" v-if="canOperate()" min-width="180">
          <template slot-scope="scope">
            ...
            <template v-if="scope.row.configType === 'CFG'">
            ...
            <el-upload class="avatar-uploader" action="" :http-request="importTable" :show-file-list="false"
              ...
              <!--<el-button  type="primary" style="margin-right:20px;" @click="getId(scope.row)">导入</el-button>-->
              <a href="javascript:;" @click="getId(scope.row)" class="operate-a">导入</a>
            </el-upload>
            <!--</el-form-item>-->
            <a :href="'/api/maai/point/export?tableId='+scope.row.id+'&tableName='+scope.row.name"
...
<script type="module">
  ...
  import {listEntity, submitPatrolTable, getPatrolKeyPointPage} from "api/medi/patrol/project";
  ...
  export default {
    async mounted() {
      this.loading = true;
      // this.page.pageNum = 1;
      plan.postTable(this.page).then((result) => {
        this.loading = false;
    ...
    methods: {
      ...
      //下载要点表导入模板
      downTableTemplet() {
        location.href = `/api/maai/point/downTableTemplet`;
      },
      //todu
      canOperate() {
      ...
      /* 添加弹出 */
      showAdd() {
        plan.postEntitytype().then((results) => {
          this.entityTypeList = results.data;
        })
        plan.postInstrurank().then((results) => {
          this.instruranklist = results.data;
        })
      ...
      addPatrolTable() {
        this.$refs.patrolTable.validate(function (data) {
          if (data) {
            this.confirm = true;
            plan.postUpdate(this.patrolTable).then(result => {
      ...
      ifEnable(tableData, enable, enableStr) {
        this.$confirm('是否确认' + enableStr + '该巡查项目？', '提示', {
          ...
          plan.postIsenable(params).then(result => {
            if (result && result.status) {
              this.$message({
      ...
      importTable(file) {
        ...
        let _this = this
        plan.importTable(formdata).then((result) => {
          this.$message({
      ...
      goContext(table) {
        this.$router.push({
          name: 'medicalDevicesGistForm.contextList',
          query: {patrolTableId: table.id, name: table.name}
        });
      },

      goDetail(table) {
        this.$router.push({name: 'medicalDevicesGistForm.DetailedList', query: {patrolTableId: table.id}});
      },
