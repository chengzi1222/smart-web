    ...
    <div class="user-box">
        ...
        <div class="content" v-if="mess.choose==0">
            <div class="specialData aloneDiv">
                ...
                <div>
                    <h3><span class="yuandian" :class="specialData.stateStr=='已完成'?'wanc':state=='执行中'?'zhix':'daiq'"></span>{{state?state:0}}</h3>
                    <p>计划状态</p>
                </div>
            </div>
            <div class="fenbuBox">
    ...
    export default {
        ...
        methods: {
            go() {
                ...
                this.$router.push({ path: '/medicalDevices/record/special/tab', query: { planName: this.$route.query.id} })
            },
            exportExcel() {
				API1.exportExcel(this.$route.query.id).then(r=>{
					if(r.status){
						downloadFile(r.data,'专项巡查计划统计报表'+this.planName)
					}else{
            ...
            getTotalPatrol() {
                API1.getTotalPatrol(this.$route.query.id).then((r) => {
                    if(!r.status){this.$message.error(r.message);return}
                    let d = r.data;
                    let name = [];
                    let result = [];
                    let jb_result = [];
            ...
            getPatrolResult() {
                API1.getPatrolResult(this.$route.query.id).then((data) => {
                    if(!data.status){this.$message.error(data.message);return};
                    // 巡查结果分布
            ...
            getProcessingType() {
                API1.getProcessingType(this.$route.query.id).then((data) => {
                     if(!data.status){this.$message.error(data.message);return};
                    // 处理方式分布
            ...
            getDatas() {//累计巡查
                API1.getSpecialData(this.$route.query.id).then((data) => {
                    if(!data.status){this.$message.error(data.message);return};
                    this.specialData = data.data;
                    if (hasAuth('maai.patrol.record')) {
                        this.isRouter = true
                    }
                })
        ...
        if(this.intervalId) {
          clearInterval(this.intervalId)
				...
				API1.getDynamic(this.$route.query.id).then(res => {
                    if(!res.status){this.$message.error(res.message);return}
            ...
            getReportSpecial() {
                API1.getReportSpecial(this.$route.query.id).then((data) => {
                    if(!data.status){this.$message.error(data.message);return}
