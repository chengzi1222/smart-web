  ...
  <div class="user-box">
    ...
    <div class="content">
      ...
      <div class="dailyData">
        <div class="dataBox">
          ...
          <div class="topDataBox aloneDiv">
            <div @click="go('maai.dailyRecord.tab')" v-if="goTwo">
      ...
      <div class="muchDataBox">
        ...
        <div style="padding: 20px;position: relative;min-height: 320px;">
          ...
          <div class="eBoxClass" :class="this.mess.chooseTwo == 3?'zIndex99':''" v-loading="loading"
               element-loading-text="拼命加载中">
            <div>
              <!-- <span style="margin: 0 20px 0 10px;">风险分级</span> -->
      ...
      <div class="specialData aloneDiv">
        <div @click="go('medicalDevices.specialList',2)" v-if="goOne">
        ...
        <div @click="go('medicalDevices.specialList',0)" v-if="goOne">
        ...
        <div @click="go('maai.specialRecord.tab')" v-if="goTwo">
...
<script type='module'>
  ...
  import * as API1 from "api/medicalDevices/superviseStatistics"
  ...
  export default {
    ...
    mounted() {
      if (this.$store.getters.auths.includes('maai.special.plan')) {
        this.goOne = true
      }
      if (this.$store.getters.auths.includes('maai.patrol.record')) {
        this.goTwo = true
      }
      ...
      this.listDictUserItemByDictCode('MAAI_QUANTIFY');//字典
    },
    methods: {
      exportExcel() {
        let params = {
        ...
        API1.excelStatisticalData(params).then(r => {
          if (r.status) {
            plan.downloadFile(r.data, '统计报表')
          } else {
            this.$message.error('请求失败导致不能导出！')
          }
        })
      },
      choose(index) {
      ...
      getSpecial() {
        API1.getStatPlanStateSpecial().then((r) => {//专项巡查概况计划数量
          if (r.status) {
      ...
      getToBeOverdueChart() {
        this.loading = true
        API1.getoverdues({'entityType': this.aboutOfEntityType}).then((data) => {
          if (!data.status) {
      ...
      getOverdueNotPatrolChart() {
        this.loading = true
        API1.getoverdues({'entityType': this.overdueEntityType}).then((data) => {
          if (!data.status) {
            this.$message.error(data.message);
            return;
          }
      ...
      getTotalPatrol() {
        this.loading = true
        API1.getPatrolTotal({'entityType': this.totalPatrolEntityType}).then((data) => {
          if (!data.status) {
            this.$message.error(data.message);
            return;
          }
      ...
      getEntityCoverageChart() {
        ...
        API1.getCoverage(params).then((data) => {
          if (data.status) {
            let xData = [];
            let coverage = [];// 主体覆盖率
            for (var i = 0; i < data.data.length; i++) {
      ...
      dynamicFn() {
        ...
        API1.dynamicsRecord(30, 0).then(res => {
      ...
      getReportDaily() {
        ...
        API1.getReportFormsDaily(params).then((r) => {
          if (!r.status) {
            this.$message.error(r.message)
          }
          this.report = r.data;
      ...
      listDictUserItemByDictCode(dictCode) {
        plan.listDictUserItemByDictCode(dictCode).then((data) => {
          ...
          this.quanLevelLis.unshift({code: '', name: "风险分级(全部)"})
        })
      }
    },
    components: {},
