  ...
  <div class="user-box" style="height: auto;margin-bottom:120px;">
    ...
    <div class="right-info">
      ...
      <el-form :model="planData" :rules="rules2" ref="planData" label-width="110px">
        ...
        <toggle-form title="计划内容">
          <div class="content">
           ...
           <div class="row particularly">
            ...
            <div class="row">
              <el-form-item label="风险分级:" prop="quantifyLevel">
                <el-checkbox-group v-model="planData.quantifyLevel" size="large">
                  <el-checkbox-button v-for="(item,index) in initData.quantifys"  :label="item.code" :key="item.code">{{item.name}}</el-checkbox-button>
                </el-checkbox-group>
              </el-form-item>
            </div>
            <div class="row">
              <el-form-item label="计划描述:" prop="desz" >
        ...
        <toggle-form title="任务分配" required>
          ...
          <el-table :data="initData.executesArray"  v-loading="loading" element-loading-text="数据正在努力加载中"   :border="true" :resizable="true" ref="multipleTable" @selection-change="selectRow" style="width: 100%;margin:30px 0 160px;">
    ...
    <div class="foot-btn" style="z-index:1" v-if="addOrEdit == 'ADD'">
      <div>
        ...
        <!--<el-button type="primary" style="width:120px;" @click="addStart">保存并启动</el-button>-->
      </div>
    </div>
    <div class="foot-btn" style="z-index:1" v-if="addOrEdit == 'EDIT'">
      <div>
        ...
        <!--<el-button type="primary" style="width:120px;" @click="editStart">保存并启动</el-button>-->
      </div>
    </div>
            ...
            <div class="row">
              <el-form-item label="主体类型:" prop="bizTypeIndex">
                <el-select v-model="tempData.bizTypeIndex" placeholder="请选择" @change="bizTypeChange($event)">
                  <el-option v-for="(item,index) in initData.bizTypes" :key="item.bizType" :label="item.bizTypeName" :disabled=item.disabled :value="index">
                  </el-option>
                </el-select>
...
export default {
  ...
  async mounted() {
   ...
   await getAreaLevel().then(r => {//判断是否==》区==》相关处理
    ...
    //主体类型列表
    await api.loadBizType().then(r => {
      this.initData.bizTypes = r.data;
    });
    //风险分级列表
    await api.loadQuantify().then(r => {
      this.initData.quantifys = r.data;
    });
    if (opt == 'add') {
      this.addOrEdit = "ADD";
      ...
      //  添加状态下，风险分级默认全选中
      let quantiy = this.initData.quantifys;
      if (quantiy && quantiy.length > 0) {
        // 风险分级默认全选中
        for (let i = 0; i < quantiy.length; i++) {
          this.planData.quantifyLevel.push(quantiy[i].code);
        }
      }
    } else if (opt == 'edit') {
      this.editInit();
  ...
  data() {
    return {
      ...
      rules2: {
        ...
        quantifyLevel: [
          { required: true, message: '请选择风险分级', trigger: 'change' }
        ],
      },
      rules: {
  ...
  methods: {
    ...
    async editInit() {//编辑初始化
     ...
     this.planData.quantifyLevel = data.planInfo.quantifyLevel ? data.planInfo.quantifyLevel.split(",") : [];
      this.planData.patrolMain = [];
      //处理主体类型Label
      if (data.patrolTableList && data.patrolTableList.length > 0) {
        for (let i = 0; i < data.patrolTableList.length; i++) {
          let labels = {
            type: data.patrolTableList[i].biz_type_name,
            project: data.patrolTableList[i].name,
            typeKey: data.patrolTableList[i].biz_type,
        ...
        if (valid) {
          _this.$confirm('是否确认更新该计划?', '提示', {
            ...
            let obj={
              ...
              'quantifyLevels': _this.planData.quantifyLevel,
            ...
            api.planspecial_update(obj).then((result) => {
              ...
              setTimeout(() => {
               _this.$router.push({name: "medicalDevices.specialList", params: {index:_this.$route.query.index}});
              }, 2000);
            });
          }).catch(_ => {
          });
    ...
    async bizTypeChange(e) {
      ...
      let formdata=new FormData();
      formdata.append('bizType', bizTypeSelects[e].bizType);
    ...
    bizAdd() {
      this.$refs["tempData"].validate(function (valid) {
        if (valid) {
          ...
          _this.planData.bizType.push(bizTypeData.bizType);
          _this.planData.patrolMain.push(projectData.id);
          let labels = {
            type: bizTypeData.bizTypeName,
            project: projectData.name,
            typeKey: bizTypeData.bizType,
            projectId: projectData.id
          };
          _this.bizTypeAndProject.push(labels);
          _this.valid.bizType = false; //如果成功选择了主体类型和巡查要点，验证提示就不显示
          _this.close();
        }
      });
      ...
      if (bizTypeSelects && bizTypeSelects.length > 0) {
        for (let i = 0; i < bizTypeSelects.length; i++) {
          for (let j = 0; j < this.planData.bizType.length; j++) {
            if (bizTypeSelects[i].bizType == this.planData.bizType[j]) {
