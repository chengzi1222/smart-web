<template>
  <baidu-map class="bm-view" :ak="baiduMapAk" @moveend="zoomend" :center="centerComputed" :zoom="zoomComputed" :min-zoom="zoomComputed-2" :map-click="false" :max-zoom="map.maxZoom" :scroll-wheel-zoom="map.scrollWheelZoom" @load="load" @zoomend="zoomend" @ready="handler">
    <entity-info-overlay class="one" :position="entityInfo.position" :active="entityInfo.active" :info="entityInfo.data" @mouseover.native="active = true" @mouseleave.native="active = false">
    </entity-info-overlay>

    <charts-box  class="two" :index='$route.query.index'></charts-box>
    <search-box class="three" @show="searchShow" @goLocation_='goLocation_area'></search-box>

    <area-overlay class="fouer" v-for="item in overlays" :key="item.areaCode" :area-name="item.areaName" :num="item.num" :lng="item.lng" :lat="item.lat"></area-overlay>
    <div v-if='is'  class="five">
      <entity-point  v-for="item in points" :key="item.id" :active-id="activeId" @actived="entityActived" :point="item"></entity-point>
    </div>
    <bm-view class="six" style="width: 100%; height:100%; flex: 1"></bm-view>
  </baidu-map>
</template>
...
import { getPaging, getChilareaList } from 'api/medicalDevices/subMap'
...
export default {
  ...
  mounted() {
    ...
    getPaging(0, 10000000, '', '', this.maaiType, '').then((data) => {
        if (data.status) {
  ...
  methods: {
    ...
    locationSearch(dataCode) {
      ...
      if (this.currentLevel > 4) {
        ...
        this.is=false;
        getChilareaList(level).then((data) => {
