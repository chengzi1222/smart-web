<template>
	<div style="position: relative;">
        ...
        <div class="ml250" >
            <div v-if="type === 'add'">
                <span>今日已提交经营主体数: {{toDaySubmitEntityCount}}</span>
            </div>
            <!-- 基础信息 -->
    ...
    methods: {
        submit(){
            Promise.all(['basics', 'principal', 'duty', 'card'].map(item => (
                ...
                if (this.type === 'add') {
                    entity.checkCreditCode('MAAI_MANAGER', this.form.creditCode).then(r => {
                        if (r.status) {
                            if (!r.data) {
