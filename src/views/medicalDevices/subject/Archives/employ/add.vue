<template>
	<div style="position: relative;">
        ...
        <div class="ml250" >
            <div v-if="type === 'add'">
                <span>今日已提交使用主体数: {{toDaySubmitEntityCount}}</span>
            </div>
            <!-- 基础信息 -->
            <BasicsInfo @getData="getData" :entityType="entityType"  :initData="form" ref="basics" :outs="out"></BasicsInfo>
            <!-- 经营信息 -->
            <ManageInfo  @getData="getData" :entityType="entityType" :type="type" :initData="form" ref="principal" :outs="out"></ManageInfo>
            <!-- 责任落实 -->
...
export default {
    ...
    mounted() {
        ...
        if (!this.entityId) {
            this.type = 'add'
            entity.getToDaySubmitEntityCount('MAAI_EMPLOY').then(r => {
    ...
    methods: {
        submit(){
            Promise.all(['basics', 'principal', 'duty'].map(item => (
                this.$refs[item].submitSub()
            ))).then(() => {
                if (this.type === 'add') {
                    entity.checkCreditCode('MAAI_EMPLOY', this.form.creditCode).then(r => {
                        if (r.status) {
                            if (!r.data) {
