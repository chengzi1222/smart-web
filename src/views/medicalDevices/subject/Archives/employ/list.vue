<template>
  <div class="user-box">
    <div class="page-tilt">
      <b></b>
      <span>医疗器械使用主体档案</span>
    </div>
    <div class="ipt-btn-box">
      <el-input placeholder="社会信用代码、名称、联系人、场所等" class="seekIpt" style="width: 180px" v-model="searchMap.keysWork">
      ...
      <el-select multiple clearable class="ml20 w150" :collapse-tags="true" v-model="searchMap.category" placeholder="使用类别" @change="pageSearch">
      ...
      <el-select multiple clearable class="ml20" style="width: 170px"  :collapse-tags="true" v-model="searchMap.enterpriseForm" placeholder="企业形态" @change="pageSearch">
        <el-option v-for="item in enterpriseFormList" :key="item.code" :label="item.name" :value="item.code">
        </el-option>
      </el-select>
        <el-button type="primary" class="w100 ml20 floatR"  @click="$router.push({name: 'medicalDevices_sub.employ.add'})">添加</el-button>
    </div>
    <div class="tabOffTop120">
      <el-table v-loading="loading" element-loading-text="数据正在努力加载中"  :border="true" :resizable="true"  @sort-change="sortChange" :cell-class-name="handleStyle" :data="tableData" class="table-div">
        ...
        <el-table-column show-overflow-tooltip prop="category" min-width="120" label="使用类别"></el-table-column>
        ...
        <el-table-column show-overflow-tooltip prop="contactPerson" min-width="120" label="联系人"></el-table-column>
        ...
        <el-table-column label="操作" min-width="250" fixed="right">
          <template slot-scope="scope">
            <a href="javascript:;" @click="$router.push({name: 'medicalDevices_sub.employ.detail', query: {entityType: scope.row.entityType, ext: 'INFO', entityId: scope.row.id}})" class="operate-a">详情</a>
            <a href="javascript:;" v-if="scope.row.optState === 'ALREADY_FILE'" v-auth="'maai.biz.eb|func.edit'"
              @click="$router.push({name: 'medicalDevices_sub.employ.detail', query: {entityType: scope.row.entityType, ext: 'CHANGE', entityId: scope.row.id}})" class="operate-a">编辑</a>
              <a href="javascript:;" v-auth="'maai.biz.eb|func.delete'" v-if="scope.row.optState === 'ALREADY_FILE'" @click="show(scope.row.id)" class="operate-a">删除</a>
          </template>
        </el-table-column> 
      </el-table>
