    ...
    <div class="content min-food">
      <div class="row">
        ...
        <el-form-item prop="grid">
          <el-select @change="gridchange" v-model="importInformaion.grid" placeholder="请选择社区网格">
            <el-option
      ...
      <div class="row">
        <el-form-item label="配置文件:">
          <el-upload class="avatar-uploader" action=""  :disabled="configDisabled"  :http-request="uploadConfig" :show-file-list="false"
          ...
          <span
            style="white-space:nowrap;position:absolute;left:100px;top:-8px;max-width:316px;color: #A2AAB6;font-size:12px;margin-top: 10px;">文件格式：xls (必填)</span>
      ...
      <div class="row" style="text-align:right">
        <el-form-item>
          <el-upload class="avatar-uploader" action=""  :disabled="disabled"  :http-request="uploadImportFile" :show-file-list="false"
                     :on-error="fileUploadError" :before-upload="beforeAvatarUpload">
            <el-button :disabled="disabled" type="primary">导入</el-button>
          </el-upload>
          <span
            style="white-space:nowrap;position:absolute;left:100px;top:-8px;max-width:316px;color: #A2AAB6;font-size:12px;margin-top: 10px;">文件格式：xls </span>
        </el-form-item>
      </div>
    </div>
  </el-form>
...
<script>
  ...
  import {getConfigFile, getSubordinate, relation, getDept, removeFile, add} from 'api/medicalDevices/importentity'
          ...
          if (this.countySelect[i].id === county) {
            getConfigFile(this.countySelect[i].areaCode, this.entityType).then(r => {
              ...
              if (data) {
                ...
                this.uploadFile = []
                this.uploadFile.push(Object.assign({}, {
            ...
            getSubordinate(this.syjSelect[i].areaCode, this.entityType).then(r => {
              ...
              for (var j = 0; j < this.gridSelect.length; j++) {
                if (this.gridSelect[j].default) {
                  ...
                  this.gridchange(this.importInformaion.grid);
                  break
                }
              }
            })
        ...
        uploadFile(formdata, {type: 'DEPT'}, callback).then((result) => {
          if (result.status) {
            ...
            relation(data.id, this.entityType).then(r => {
              if (r.status) {
                _this.uploadFile = []
                _this.uploadFile.push(Object.assign({}, {
                  fileName: data.fileName,
                  type: data.type,
                  id: data.id,
                  fileId: data.fileId,
