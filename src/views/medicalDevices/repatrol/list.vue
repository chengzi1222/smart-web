  ...
  <div class="bigbox">
    ...
    <div>
      <el-input :placeholder="placeholder" class="w200 mr20 seekIpt"
                v-model="searchMap.searchStr" @keyup.enter.native="getPageData">
        <i slot="suffix" class="el-input__icon el-icon-search" @click="getPageData"></i>
      </el-input>
      <el-select @change="getPageData" collapse-tags multiple v-model="searchMap.entityTypeCond"
      ...
      <el-select @change="getPageData" collapse-tags multiple v-model="searchMap.resultCond"
      ...
      <el-select @change="getPageData" collapse-tags multiple v-model="searchMap.treatCond"
    ...
    <div :class="down?'tabOffTop145':'tabOffTop170'">
      <el-table v-loading="loading_table" element-loading-text="数据正在努力加载中" :border="true" key='ok'
        ...
        <el-table-column show-overflow-tooltip prop="sourceResultStr" min-width="80"
        ...
        <el-table-column show-overflow-tooltip label="操作" min-width="100">
          <template slot-scope="scope">
            ...
            <a @click="goAdd(scope.row);" v-auth="'maai.repatrol.record|func.add'" href="javascript:;"
  ...
  export default {
    methods: {
      ...
      choose(index, type) {
        ...
        this.searchMap.searchStr = ''
        this.searchMap.entityTypeCond = []
      ...
      goAdd(row) {
        this.$router.push({
          name: 'medicalDevices.RepatrolRecordAdd',
          query: {
            'repatrolRecordId': row.id,
            "messType": this.searchMap.state,
      ...
      goDetail(row, t) {
        this.$router.push({
          name: 'medicalDevices.RepatrolRecordInfo',
          query: {
            'repatrolRecordId': row.id,
            "messType": t,
            'isList': 'YES'
          ...
          this.$router.push(
            {
              ...
              query: {recordId: row.sourceId, patrolType: "DAILY", patrolAreaCode: row.sourcePatrolDept}
            })
        } else if (type === '整改复查') {
          this.$router.push(
            {
              name: 'medicalDevices.RepatrolRecordInfo', query: {
                'repatrolRecordId': row.id,
                'messType': t,
                'isList': 'YES'
              }
          ...
          this.$router.push(
            {
              ...
              query: {recordId: row.sourceId, patrolType: "SPECIAL", patrolAreaCode: row.sourcePatrolDept}
            })
        }
      },
      ...
      async printInitData(row, type) {
        ...
        let id = row.id;
        //基础信息请求参数
        await api.getBascInfo({'id': id,state:'END','queryType': 'DETAIL',}).then(d => {
          if (d.status && d.data) {
            let baseInfo;
            baseInfo = d.data;
            this.basicInfoData.entityName = baseInfo.entityName;
            this.basicInfoData.creditCode = baseInfo.creditCode;
            this.basicInfoData.spaceAddr = baseInfo.spaceAddr;
            this.basicInfoData.corpName = baseInfo.legalPerson;
            this.basicInfoData.mobile = baseInfo.contactMobile;
            ...
            this.resultData.recordNo = baseInfo.repatrolRecordNo
        ...
        await api.getResultInfo({'id': id,'state':'END','queryType': 'DETAIL',}).then(d => {
          if (d.status && d.data) {
            let resultInfo;
            resultInfo = d.data;
    ...
    mounted() {
      if (this.$route.query.type) {
        ...
        this.searchMap.state = this.$route.query.type;
      }
