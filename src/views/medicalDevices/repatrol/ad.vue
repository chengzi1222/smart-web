...
<template>
  <div v-loading="loading" element-loading-text="拼命加载中"
            ...
            <div class="content">
              ...
              <div class="row">
                <el-form-item :label="patrolRecordNo_">
                  <span
                    :key="curd + '-repatrolRecordNo'">{{baseInfo.repatrolRecordNo ? baseInfo.repatrolRecordNo  : '暂无'}}</span>
                </el-form-item>
              </div>
              <div class="row">
                <el-form-item label="社会信用代码:">
                  <span :key="curd + '-license'"> {{baseInfo.creditCode ? baseInfo.creditCode  : '暂无'}}</span>
                </el-form-item>
              ...
              <div class="row">
                ...
                <el-form-item label="巡查结果:">
                  <span
                    :key="curd + '-sourceResultLabel'">{{baseInfo.sourceResultStr ? baseInfo.sourceResultStr  : '暂无'}}</span>
                </el-form-item>
              </div>
            ...
            <div class="mT35" v-if="tableList">
              <el-table v-for="(item,index) in tableList" :key="item.id"
                ...
                <el-table-column align="center" label="备注" min-width='130'>
                  <template slot-scope="scope">
                    <input type="text" class="w120" v-model="scope.row.remark" :maxlength="30"
                           :disabled="!scope.row.suit" placeholder="备注前先选择是否符合">
                  </template>
                </el-table-column>
                <el-table-column align="center" label="附件" min-width='80'>
                  <template slot-scope="scope">
                    <el-upload
                      ...
                      :data="{type:'MAAI_REPATROL',field:'PHOTO'}"
            ...
            <div class="content">
              ...
              <div class="row">
                <el-form-item label="上传图片：" prop="">
                  <el-upload action="/api/upload" :data="{type:'MAAI_REPATROL', field: 'PHOTO'}"
              ...
              <div class="row mRno">
                ...
                <el-upload
                  ...
                  :on-success="handleSignSuccess"
                  :on-error="uploadError"
                  :data="{type:'MAAI_REPATROL', field: 'SIGN'}"
                  :before-upload="beforeSignUpload"
                  :on-preview="handlePictureCardPreview"
                  :on-remove="handleRemove"
                  :on-exceed='maxLimit1'
    ...
    <yb-popup @close="close" :isShow="isShow" title="操作提示" width="350">
      ...
      <div slot="foot">
        <el-button type="defalut"
                   @click="$router.push({name: 'medicalDevices.RepatrolRecordList',query: {'type':this.messType}})">返回
        </el-button>
    ...
    <div class="foot-btn" style="z-index:1">
      <div>
        <el-button @click="$router.push({name: 'medicalDevices.RepatrolRecordList', query: { 'type': messType}});;"
...
<script type="module">
  ...
  export default {
    ...
    async mounted() {
      ...
      let infoObj
        infoObj = {'id': this.repatrolRecordId, 'queryType': 'REPATROL','state':'TODO'}
    ...
    methods: {
      ...
      updateRepatroRecord() {
        this.$refs['repatrol'].validate((valid) => {
          ...
          this.initContext.contextList.forEach(item => {
            item.context.forEach(context => {
              if (context.suit !== undefined && context.suit !== '') {
            ...
            this.$confirm('是否要提交复查记录?', '提示', {
              ...
              this.initContext.contextList.forEach(item => {
                item.context.forEach(context => {
                    ...
                    obj.suit = context.suit=='NO'? context.suit:'YES'
      ...
      singlecheckboxchange(args) {
        this.$set(args.row, 'suit', args.result);
        this.contextStat = {
      ...
      goPatrolDetail() {
        if (this.baseInfo.sourceTypeStr === '日常巡查') {
          this.$router.push(
            {name: "maai.dailyRecord.info", query: {recordId: this.baseInfo.sourceId, patrolType: "DAILY",patrolAreaCode:this.baseInfo.sourcePatrolDept}})
          ...
          this.$router.push(
            {name: 'maai.specialRecord.info', query: {recordId: this.baseInfo.sourceId, patrolType: "SPECIAL",patrolAreaCode:this.baseInfo.sourcePatrolDept}})
        }
      },
      //跳详情
      goDetail() {
