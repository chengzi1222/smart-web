<template>
  <div v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading"
    ...
    <div style="margin-top:20px;border-left:3px solid  #0DB5EF;">
      <span style="margin-left:10px;font-size: 16px;color: #393939;font-weight:bolder;">复查</span>
    </div>
    <div class="container">
      <el-form :model="repatrol" :rules="rules" ref="repatrol" label-width="140px" class="demo-ruleForm">
          ...
          <div class="content">
            ...
            <div class="row">
              <el-form-item label="申请类型:">
                <span>{{baseInfo.applyTypeStr ? baseInfo.applyTypeStr  : '暂无'}}</span>
              </el-form-item>
            ...
            <div class="row">
              <el-form-item label="业务受理号:">
                <span>{{baseInfo.businessNo ? baseInfo.businessNo  : '暂无'}}</span>
              </el-form-item>
            </div>
            <div class="row">
              <el-form-item label="核查时间:">
                <span>{{baseInfo.sourceTime ? baseInfo.sourceTime  : '暂无'}}</span>
              </el-form-item>
              <el-form-item label="整改截止日期:">
                <span>{{baseInfo.sourceRectifyTime ? baseInfo.sourceRectifyTime  : '暂无'}}</span>
              </el-form-item>
            </div>
        ...
        <!-- 巡查项 -->
        <toggle-form title="现场复查项（仅显示本次复查的核查项）">
            ...
            <el-table v-for="(item,index) in tableList" :key="item.id" :class="index==0?'firsTeit':''" :data="item"
              ...
              <el-table-column align="center" label="是否符合(必填)" min-width="250">
                <template slot-scope="scope">
                  <single-checkbox :currentRowData="scope.row" @singlecheckboxchange="singlecheckboxchange">
                  </single-checkbox>
                </template>
              </el-table-column>
        ...
        <toggle-form title="巡查结果">
          <div class="content">
            ...
            <div class="row ml30 mb20 textC" style="color: #6D7787;">
              判定标准：关键项不符合数：≤{{baseInfo.standardNoCruxNum}}项，重点项不符合数：≤{{baseInfo.standardNoImportantNum}}项，一般项+重点项不符合数：≤{{baseInfo.standardNoGeneralAndNoImportantNum}}项
            </div>
            <div class="row bottomContent">
              <el-form-item label="复查结果：" :required='true'>
            ...
            <div class="row bottomContent">
              <el-form-item label="处理方式：" prop="treat">
                <el-radio-group v-model="repatrol.treat" size="medium" @change="rulesChange('treat')"
                  ...
                  <el-radio-button label="RE_RECTIFY" v-if="baseInfo.repatrolCount<=1">再次限期整改</el-radio-button>
            ...
            <div v-if="repatrol.treat === 'RE_RECTIFY'" class="row bottomContent">
              <el-form-item label="整改期限：" prop="rectifyTime" :required='true'>
                <el-date-picker class="w200" format="yyyy 年 MM 月 dd 日" value-format="yyyy-MM-dd"
                  v-model="repatrol.rectifyTime" type="date" placeholder="选择整改期限" :picker-options="pickerOptions">
                </el-date-picker>
              </el-form-item>
            </div>
            <div class="row bottomContent">
              <el-form-item label="复查人：" prop="inspectorUserId">
                <el-select v-model="repatrol.inspectorUserId" class="w250 " placeholder="请输入搜索复查人员" filterable remote
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      api.repatrolWaitDetail(this.repatrol.id).then(d => {
        if (d.status) {
          this.baseInfo = d.data;
          let datas = d.data.projects;
          this.tableList = [];
          this.contextStat.contextCount = 0;
          this.contextStat.impor3Count = this.baseInfo.standard.currentCruxNum;
          this.contextStat.impor2Count = this.baseInfo.standard.currentImportantNum;
          this.contextStat.impor1Count = this.baseInfo.standard.currentGeneralNum;
          datas.map(item => {
            let lis = []
    ...
    methods: {
        ...
        this.$refs['repatrol'].validate((valid) => {
          ...
          if (valid) {
            this.$confirm('是否要提交复查记录?', '提示', {
              ...
              api.repatrol(this.repatrol).then(result => {
      ...
      singlecheckboxchange(args) {
        ...
        let result3 = this.contextStat.unImpor3Count <= this.baseInfo.standardNoCruxNum
        let result2 = this.contextStat.unImpor2Count <= this.baseInfo.standardNoImportantNum
        let result1 = this.contextStat.unImpor1Count + this.contextStat.unImpor2Count <= this.baseInfo
