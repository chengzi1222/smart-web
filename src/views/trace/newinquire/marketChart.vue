...
<script>
  ...
  import { bar_echart } from "./echarts/echarts";
  import * as api from "api/trace/inquire";
  import { getTraceWarning, getTraceUse, getTraceTrend } from 'api/trace/newfarm'
  export default {
    components: {
      Back
    },
      ...
      getTraceUse() {
        ...
        getTraceUse(params).then(r => {
          if (r.status) {
            ...
            bar_echart(this.useData, "IMusearea", this.$router);
          }
        })
      },
      getTraceTrend() {
        ...
        getTraceTrend(params).then(r => {
          if (r.status) {
            ...
            bar_echart(this.hisData, "IMhis", this.$router);
          }
        })
      },
      //  获取时间处理，默认为当前时间
