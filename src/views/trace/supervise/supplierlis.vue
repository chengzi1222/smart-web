<template>
    <div class="bigbox">
        <div class="page-tilt">
            <b></b><span>配送商列表</span><Back></Back>
        </div>
        <div>
            <el-input placeholder="请输入配送商名称" class="w260 seekIpt" v-model="subName" @keyup.native="seekEnter">
        ...
        <div style="position: relative;">
          <div v-if="lodings" style="position: absolute;top: 0;left: 0;bottom: 0;right: 0;z-index: 99;text-align: center;background-color: rgba(0,0,0,.1);">
            ...
            <el-table :data="dataPage" class="table-div pointerTab">
                ...
                <el-table-column show-overflow-tooltip prop="subName" min-width="150" label="配送商名称"></el-table-column>
                <el-table-column show-overflow-tooltip prop="address" min-width="150" label="地址"></el-table-column>
                ...
                <el-table-column show-overflow-tooltip min-width="120" label="查看资质">
                    <template slot-scope="scope">
                        <span style="width: 100%;display: inline-block;" @click="go(scope.row)">查看</span>
                    </template>
                </el-table-column>
            </el-table>
...
export default {
    methods: {
        ...
        go(row){
            this.$router.push({
                name:'yb.aptitudeLook',
                query:{
                    bslicImg:row.bslicImg
                }
            })
        },
        elseDown(){
        ...
        down(){
            this.derive = encodeURI(config.Derive + 'govapi/sub/exportRelation?guid=' + this.guid + '&subName=' + this.subName + '&type=buyer');
        }
    },
    mounted() {
