<template>
  <div class="user-box">
    <div class="page-tilt">
      <b></b>
      <span>农贸市场溯源监管 - 综合查询分析</span><Back></Back>
    </div>
    <ul class="tab-box">
    ...
    <div class="ipt-btn-box">
      <div class="iptsbox" v-if="areaIf">
        ...
        <el-select class="w150 mr10" @change="areaIdChange" v-model="areaIdNum" placeholder="请选择">
          <el-option v-for="item in areaIdLis" :key="item.areaId" :label="item.areaName" :value="item.areaId">
          </el-option>
        </el-select>
      </div>
      <div class="iptsbox" v-if="mess.key != 1">
        ...
        <el-select class="w150 mr10" @change="cxClick" filterable v-model="marketId" placeholder="请选择">
          <el-option v-for="item in marketIdLis" :key="item.compid" :label="item.compName" :value="item.compid">
          </el-option>
        </el-select>
      </div>
    ...
    <div style="position: relative;">
      ...
      <el-table :data="tableData" class="table-div" :key="mess.choose">
        ...
        <el-table-column show-overflow-tooltip prop="baseName" label="供货商" v-if="mess.key == 0"></el-table-column>
    ...
    <div class="popup-box" v-if="isShow">
      <div class="popup-div">
        ...
        <div style="width:500px;height:400px;">
          <img :src="image" style="width: 100%;height: 300px;background-color: #ccc;">
        </div>
  ...
  export default {
    ...
    mounted() {
      ...
      if (this.$store.state.common.user.areaCode.length == 4) {
        this.areaId = this.$store.state.common.user.regionCode.slice(0, 4);
        this.areaType = this.$store.state.common.user.regionCode.slice(0, 4);
      } else {
        this.areaId = this.$store.state.common.user.regionCode;
        this.areaType = this.$store.state.common.user.regionCode;
      }
    ...
    methods: {
      ...
      choose(index, key) {
        ...
        if (this.$store.state.common.user.areaCode.length == 4) {
          ...
          this.areaIdNum = this.$store.state.common.user.regionCode.slice(0, 4);
        } else {
          this.areaId = this.$store.state.common.user.regionCode;
          this.areaType = this.$store.state.common.user.areaCode;
      ...
      getLoadRetailMarketInfo() {
        ...
        plan.getLoadRetailMarketInfo(form).then(d => {
          this.marketIdLis = d;
        })
      },
      //溯源详情查询
      getLoadRetailInitSource() {
        ...
        plan.getLoadRetailInitSource(form).then(d => {
          this.lodings = false;
      ...
      getLoadLowSourceArea() {
        ...
        plan.getLoadLowSourceArea(form).then(d => {
          this.lodings = false;
          this.tableData = d.rows;
          this.page.total = d.total;
        })
      ...
      getLoadLowSourceMarket() {
        ...
        plan.getLoadLowSourceMarket(form).then(d => {
          this.lodings = false;
          this.tableData = d.rows;
          this.page.total = d.total;
        })
      ...
      getLoadLowSourceBiz() {
        ...
        plan.getLoadLowSourceBiz(form).then(d => {
          this.lodings = false;
          this.tableData = d.rows;
          this.page.total = d.total;
        })
