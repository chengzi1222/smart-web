  ...
  methods: {
    ...
    down(){
      if(this.seldate.length == 7){
        this.derive = encodeURI(config.export + '/api/v1/stat/retailer/dc/rank/street/export?areaId=' + this.areaId_ + '&date=' + this.seldate);
      } else{
        this.derive = encodeURI(config.export + '/api/v1/stat/retailer/dc/rank/street/export?areaId=' + this.areaId_ + '&date=' + this.getdate(this.seldate));
      }
    },
    rowclick(row, event) {
