<template>
    <div class="area">
        <div class="page-tilt">
            <b></b><span>农贸市场溯源监管{{ title == '' ? '': (title[0]=="-" ? title :"-"+title ) }}</span><Back v-show="backshow"></Back>
        </div>
        ...
        <div v-if="rankList" class="rankBox">
            <div class="rank">
                <h3>区县使用率排行<a :href="derive" class="w65 dc godown" target="_blank" download="filename" style="display: inline-block">导出</a><span style="float: right;margin-right: 10px;cursor: pointer;" @click="ranking">X</span></h3>
  ...
  methods: {
    ...
    down(){
      if(this.seldate.length == 7){
        this.derive = encodeURI(config.export + '/api/v1/stat/retailer/dc/rank/areas/export?areaId=' + this.areaId_ + '&date=' + this.seldate);
      } else{
        this.derive = encodeURI(config.export + '/api/v1/stat/retailer/dc/rank/areas/export?areaId=' + this.areaId_ + '&date=' + this.getdate(this.seldate));
      }
    },
    use_() {
       this.down();
