<template>
  <div class="tablePrint" v-show="printShow">
    <div id="printHead">
      <p class="print_w100 print_titie1">{{ resultData.taskEntity?resultData.taskEntity.task_name:"" }}</p>
      <p class="print_w100 print_titie2">食品安全保障每日现场记录表</p>
      <p style="margin-top: -10px" v-if="resultData.patrolDetail">编号：{{ resultData.patrolDetail.patrol_no }} &nbsp; &nbsp;
        <span id="printPage" style="display:none">第1页，共X页</span></p>
    </div>
    <div id="printBody" printBodyTop="155" intHeight="80%">
      <table class="print_w100 print_table" border="1" cellspacing="0" cellpadding="0">
        <tr class="" v-if="resultData.taskEntity">
          <td colspan="3" class="thead_t">餐饮单位名称（量化等级）： &nbsp; {{resultData.taskEntity.entity_name}} &nbsp;（{{resultData.quanLevel}}）</td>
        </tr>
        <tr>
          <td class="thead_t w10"> 编号</td>
          <td class="thead_t w65" style="text-align: left;">重大活动现场检查项目</td>
      ...
      <p style="margin-top:20px;margin-bottom:5px;width:100%;" v-if="resultData.patrolDetail">
        <span style="width: 50%;display: inline-block;">监督人员：&nbsp;
          <img :src="pathFile + resultData.patrolDetail.so_sign" alt="" style="max-width: 200px;width: auto;max-height: 200px;height: auto">
        </span>
        接待单位负责人：&nbsp;
        <span v-if="resultData.patrolDetail.is_cooperate === 'NO'">企业拒绝配合</span>
      ...
      <p>监督日期：{{resultData.patrolDetail?resultData.patrolDetail.patrol_date:""}}</p>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      let title2 = content.getElementsByClassName("print_titie2");
      for (let i = 0; i < title1.length; i++) {
        ...
        title2[i].style.fontSize = "16px";
        title2[i].style.paddingBottom = "20px";
        title2[i].style.fontFamily = "微软雅黑";
      }
      ...
      let left_f = content.getElementsByClassName("left_f");
      for (let i = 0; i < left_f.length; i++) {
        left_f[i].style.float = "left";
        left_f[i].style.width = "60px";
      }
      let zfx = content.getElementsByClassName("zfx");
      for (let i = 0; i < zfx.length; i++) {
        zfx[i].style.width = "10px";
        zfx[i].style.height = "10px";
        zfx[i].style.borderColor = "#000";
        zfx[i].style.borderStyle = "solid";
        zfx[i].style.borderWidth = "1px";
        zfx[i].style.display = "inline-block";
      }
      let over = content.getElementsByClassName("over");
      for (let i = 0; i < over.length; i++) {
        over[i].style.overFlow = "hidden";
      }
      let gou = content.getElementsByClassName("gou");
      for (let i = 0; i < gou.length; i++) {
        gou[i].style.positon = 'relative';
        gou[i].style.left = '20px';
      }
    },
    data() {
      return {
