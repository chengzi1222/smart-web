...
<template>
  <div class="user-box" style="height: auto;margin-bottom:120px;">
    ...
    <div class="right-info">
      <el-form :model="planData" :rules="rules" ref="planData" label-width="110px">
        <!-- 计划内容 -->
          ...
          <div class="content">
            ...
            <el-form-item label="计划时间:" required>
              <el-col :span="7">
                <el-form-item prop="startDate">
                  <el-date-picker v-model="planData.startDate" class="w200" format="yyyy-MM-dd" type="date" :editable="false" placeholder="选择日期"
              ...
              <el-col :span="7">
                <el-form-item prop="endDate">
                  <el-date-picker v-model="planData.endDate" class="w200" format="yyyy-MM-dd" type="date" placeholder="选择日期" :editable="false"
            ...
            <span class="row">
              <el-form-item label="相关文件:" required>
                ...
                <span style="white-space:nowrap;position:absolute;left:100px;top:-8px;max-width:316px;color: #A2AAB6;font-size:12px;margin-top: 10px;">最多上传5个文件，文件格式：jpg，png，doc，pdf
                  (选填)</span>
        ...
        if (this.addOrEdit == "ADD") {
          if (this.planData != null) {
            this.$confirm('是否确定放弃添加专项巡查计划？', '提示', {
              ...
              this.$router.push({ name: "mediSpecial", params: { index: this.$route.query.index } });
            });
          }
        ...
        this.$refs["planData"].validate(function (valid) {
          if (valid) {
            _this.$confirm('是否确认更新该计划?', '提示', {
              ...
              // 将时间类型转为字符串
              _this.planData.startDate = _this.getdate(_this.planData.startDate);
              _this.planData.endDate = _this.getdate(_this.planData.endDate);
              update(_this.planData).then((result) => {
                ...
                setTimeout(() => {
                  _this.$router.push({ name: "mediSpecial", params: { index: _this.$route.query.index } });
                }, 2000);
              });
            }).catch(_ => {
            });
        ...
        this.$refs["planData"].validate(function (valid) {
          if (valid) {
            _this.$confirm('是否确认保存该计划?', '提示', {
              ...
              // _this.planData.endDate = _this.getdate(_this.planData.endDate);
              let clonePlanData = {}
              Object.keys(_this.planData).forEach(key => {
                if (key === 'startDate' || key === 'endDate') {
                  clonePlanData[key] = _this.getdate(_this.planData[key])
                } else {
                  clonePlanData[key] = _this.planData[key]
                }
              })
              addSpceial(clonePlanData).then((result) => {
                if (!result.status) {
                  _this.$message({  
                    message: result.message,
                    type: result.level
