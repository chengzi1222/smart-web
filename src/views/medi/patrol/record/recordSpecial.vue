<template>
  <div v-loading="loading" element-loading-text="拼命加载中"
    ...
    <div class="ipt-btn-box">
      <el-input style="vertical-align:top;" placeholder="名称、编号、人员" class="w200 seekIpt"
      ...
      <select-area-grid class="selectHeight w150" placeholder="分管网格"
                        @change="deptChange"></select-area-grid>
      <el-select clearable class="selectHeight w250" v-model="search.specialPlanIds" multiple
                 collapse-tags
                 placeholder="巡查计划（全部）"
                 @change="handleIconClick">
        <el-option v-for="item in specialPlanList" :key="item.id" :label="item.planName"
                   :value="item.id">
        </el-option>
      </el-select>
      <!-- <i :class="down?'el-icon-caret-top':'el-icon-caret-bottom'" class="down"
         @click="downSeleBox"></i> -->
      <el-button class="selectHeight floatR" type="primary" v-auth="'medi.patrol.record|func.add'"
                 @click="add"><i style="margin-right: 5px;font-weight: 700;color: #fff;"
                                 class="el-icon-plus"></i>添加记录
      </el-button>
    </div>
    <div class="ipt-btn-box" style="margin-left: -20px;">
      <enum-select v-model="search.entityType" multiple placeholder="主体类型" @change="handleIconClick"
                   enum="com.ybveg.govx.enums.medi.MediEntityTypeEnum"></enum-select>
      <enum-select v-model="search.result" multiple placeholder="巡查结果" @change="handleIconClick"
                   enum="com.ybveg.govx.enums.patrol.RecordResultEnum"></enum-select>
      <enum-select v-model="search.treat" multiple placeholder="处理方式" @change="handleIconClick"
                   enum="com.ybveg.govx.enums.patrol.TreatEnum"></enum-select>
    </div>
    <div class="tabOffTop170">
      <el-table  :border="true" :resizable="true"  :data="tableData" class="table-div" @sort-change="handleSortChange" :key="maxheight"
                 :class="shadowShow?'shadowNo':''">
        <el-table-column type="index" min-width="40" label="序号"></el-table-column>
        <el-table-column show-overflow-tooltip prop="entityName" min-width="100"
                         label="主体名称"></el-table-column>
        <el-table-column show-overflow-tooltip prop="patrolRecordNo" min-width="120"
                         label="巡查记录编号"></el-table-column>
        <el-table-column show-overflow-tooltip prop="deptName" label="辖区机构"  min-width="120"></el-table-column>
        <el-table-column show-overflow-tooltip prop="planName" min-width="150"
                         label="巡查计划"></el-table-column>
        <el-table-column show-overflow-tooltip prop="entityTypeExt" min-width="80"
                         label="主体类型"></el-table-column>
        <el-table-column show-overflow-tooltip prop="superviseName" min-width="80"
                         label="监管人员"></el-table-column>
        <el-table-column show-overflow-tooltip prop="messName" min-width="80"
                         label="信息员"></el-table-column>
        <el-table-column show-overflow-tooltip prop="result" min-width="80"
                         label="巡查结果"></el-table-column>
        <el-table-column show-overflow-tooltip resizable min-width="90" label="问题项/巡查项">
        ...
        <el-table-column show-overflow-tooltip prop="patrolTime" sortable="custom" min-width="120"
                         label="巡查日期"></el-table-column>
        <el-table-column label="操作" fixed="right" min-width="120">
          <template slot-scope="scope">
            <a href="javascript:;" @click="view(scope.row.id)" class="operate-a">详情</a>
            <a href="javascript:;" @click="printRecord(scope.row.id, true)"
               class="operate-a">打印预览</a>
            <a href="javascript:;" @click="printRecord(scope.row.id, false)"
               class="operate-a">打印</a>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  import {findListByAreaCode, findSpecialPage} from "api/medi/patrol/record";
  ...
  export default {
    components: {
      EnumSelect, SelectAreaGrid, PrintPatrol
    },
    async mounted() {
      ...
      if (this.$route.query.stateTxt) {
        ...
        this.search.startDate = dates.getFullYear() + '-01-01';
        this.search.endDate = dates.getFullYear() + '-' + mon + '-' + day
      }
      ...
      if(this.$route.query.planId){
        this.search.specialPlanIds.push(this.$route.query.planId);
    ...
    methods: {
      ...
      add() {
        this.$router.push('/medipatrol/record/special/add');
      },
      //页面查询
      async getPage() {
          ...
          d.data.forEach(list => {
            ...
            list.content.forEach(content => {
              if (content.important === 'YES') {
                ...
                if (content.qual === 'YES' || !content.qual || content.qual == null || content.qual == '') {
