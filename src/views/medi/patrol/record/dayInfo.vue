...
<template>
  <div v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(255, 255, 255, 0.8)"
        ...
        <el-form :model="ruleForm" ref="ruleForm" label-width="110px" class="demo-ruleForm">
            ...
            <div class="content" v-if="basicInfoData">
              <div class="row">
                <el-form-item label="主体名:" prop="">
                  ...
                  <!--v-auth="'food.biz.cb,food.biz.pb,food.biz.rb,food.biz.cfda'"-->
                ...
                <el-form-item label="巡查地址:" prop="">
                  ...
                  <el-dialog v-if="patrolRecordAddressIsShow" :visible.sync="mapAddressVisible" :modal-append-to-body="false" width="60%">
                    ...
                    <div style="height: 300px;">
                      <address-map v-if="patrolRecordAddressIsShow" :lng="basicInfoData.patrolRecordAddress.lng" :lat="basicInfoData.patrolRecordAddress.lat">
                      </address-map>
                    </div>
                  </el-dialog>
                </el-form-item>
              ...
              <div class="row">
                ...
                <el-form-item :label="infoTitle" prop="" v-if="$store.getters.user.areaCode.length > 6">
                  <span>{{basicInfoData.messName}}</span>
                </el-form-item>
              </div>
          ...
          <toggle-form title="巡查项（仅显示本次检查的巡查项）">
            <div class="mT35" v-if="patrolData">
              ...
              <el-table :key="index" v-for="(item,index) in patrolData" :class="index==0?'firsTeit':''" :data="item.content" :span-method="objectSpanMethod"
                border class="table-div brN">
                <el-table-column show-overflow-tooltip label="检查项编号" prop="pno" width="100"></el-table-column>
                ...
                <el-table-column show-overflow-tooltip label="是否检查通过" width="120">
                  <template slot-scope="scope">
                    {{scope.row.qual == 'YES' ? '检查通过':scope.row.qual == 'NO'?'检查不通过':'未检查'}}
                  </template>
                </el-table-column>
          ...
          <toggle-form title="巡查结果">
            <div class="content" v-if="resultData">
              <div class="row ml30" style="margin-left: 40px;">
                <span style="color: #6D7787;">本次巡查重点项<span>{{resultData.importantItem.total}}</span>项，一般项<span>{{resultData.generalItem.total}}</span>项，共{{resultData.generalItem.total
                  + resultData.importantItem.total}}项</span>
              </div>
              ...
              <div class="row ml30" style="margin: 20px 0">
                <span style="font-size: 18px;color: #393939;">企业得分：</span><span style="color: #0DB5EF;font-size: 18px; "><span>{{resultData.score
                    - noPassScore}}</span>分</span>
    ...
    <!-- 打印 -->
    <PrintPatrol ref="PrintPatrol" :basicInfoData="basicInfoDataPrint" :patrolData="patrolDataPrint"
      :patrolContent="patrolContentPrint" :resultData="resultDataPrint" :patrolType="patrolType == 'DAILY'? '日常监督抽查': '专项整治活动'"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getPatrolContext(){
        plan.getPatrolItems(this.recordId,this.patrolType,this.contextType).then(d => {
          //不合格分置0，避免后面更换查看模式累加
          this.noPassScore = 0;
      ...
      goEntityDatail() {
        ...
        this.$router.push({
          path: '/medi/subject/' + this.basicInfoData.entityType.toLowerCase() + '/detail',
          query: {
            entityType: this.basicInfoData.entityParentType,
            entityTypeExt: this.basicInfoData.entityType,
            ext: 'INFO',
            entityId: entityId
          }
        });
      ...
      async printInitData(id, type) {
        ...
        plan.getPatrolItems(id, type,this.contextType).then(d => {
        ...
        plan.getPatrolResult(datas).then(d => {
          ...
          if (this.resultDataPrint.rectifyTime) {
            if (this.resultDataPrint.rectifyTime.indexOf(" ") > 0) {
              ...
              if (rectifyTimeArr != []) {
                this.resultDataPrint.rectifyTime = rectifyTimeArr[0] + "年" + rectifyTimeArr[1] + "月"
                  + rectifyTimeArr[2] + "日";
              }
            }
          }
