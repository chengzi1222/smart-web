...
<template>
  <div>
    <div style="margin-top:20px;border-left:3px solid  #0DB5EF;">
      <span style="margin-left:10px;font-size: 16px;color: #393939;font-weight:bolder;">添加日常巡查记录</span>
    </div>
    <div class="container">
      <!-- 左边信息悬浮栏 -->
      <div class="left-info">
        ...
        <ul class="main-block">
          <li class="clemedipatrol/record/special/addarfix " :class="slideIndex==index?'activePosition':''" v-for="(item,index) in progress" :key="index">
      ...
      <div class="right-info">
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="120px" class="demo-ruleForm">
            ...
            <div class="content">
              <div class="row">
                <el-form-item label="主体信息:" prop="">
                  <select-mainbody @slmainbody="slmainbody" :currentIndex='currentIndex' :apiFn='selectMainbodyApiFn' :entryType='"mediPatrol"'
                                   @citem="citem" :enumClass='"com.ybveg.govx.enums.medi.MediEntityTypeEnum"'></select-mainbody>
                </el-form-item>
              </div>
              <!-- 选择的主体 -->
              ...
              <div class="row">
                <el-form-item label="巡查时间:" prop="patrolTime">
                  <div class="block">
                    <el-date-picker class="w200" v-model="ruleForm.patrolTime" @change="timeChange" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择日期" :picker-options="pickerOptions0">
                    </el-date-picker>
                  </div>
                </el-form-item>
              </div>
              ...
              <div class="row">
                <el-form-item :label="$store.getters.user.areaCode.length > 6?'监管人员':'检查人员'" prop="patrolSoId">
                  <el-select @blur="blurCount" class="w200" multiple multiple-limit=5 :placeholder="$store.getters.user.areaCode.length > 6?'请选择监管人员':'请选择检查人员'" v-model="ruleForm.patrolSoId" @change="changeSo">
              ...
              <div class="row" v-if="$store.getters.user.areaCode.length > 6">
                <el-form-item :label="infoTitle" prop="infoList">
                  <el-select @blur="blurCount" class="w200" placeholder="请选择信息人员" v-model="ruleForm.infoList" multiple multiple-limit=5 collapse-tags :placeholder="infoPlaceholder" @change="changeMess">
                    <el-option v-for="item in ruleForm.info" :key="item.id" :label="item.realName" :value="item.id" :disabled="item.disabled">
                    </el-option>
                  </el-select>
                </el-form-item>
              </div>
          ...
          <toggle-form title="巡查项">
            ...
            <div v-if="patrolData&&patrolData.length>0" style="margin-top:35px;">
              <el-table :key="index" v-for="(item,index) in patrolData" :class="index==0?'firsTeit':''" :data="item.conList" :span-method="objectSpanMethod"
                ...
                <el-table-column align="center" label="附件">
                  <template slot-scope="scope">
                    <el-upload action="/api/upload" :before-upload="beforeUpPic" :on-success="picSuccess" :data="{type:'MEDI_PATROL_RECORD',field:'PHOTO'}"
            ...
            <div class="content">
              ...
              <div class="row">
                <el-form-item label="上传图片：" prop="">
                  <el-upload :file-list="photo" action="/api/upload" list-type="picture-card" :on-success="uploadSuccess" :on-error="uploadError"
                    :data="{type:'MEDI_PATROL_RECORD',field:'PHOTO'}" :before-upload="beforeAvatarUpload" :on-preview="handlePictureCardPreview"
                    :on-remove="removePic">
              ...
              <div class="row mRno">
                ...
                <el-upload :file-list="sign" v-if="ruleForm.isCooperate == 'yes'" action="/api/upload" list-type="picture-card" :on-success="uploadSuccess"
                  :on-error="uploadError" :data="{type:'MEDI_PATROL_RECORD',field:'SIGN'}" :before-upload="beforeAvatarUpload"
    ...
    <div class="foot-btn" style="z-index:1">
      <div>
        <el-button style="width:120px;" @click="$router.push({name: 'mediPatrolRecordDailyList'})">
          取消
        </el-button>
    ...
    <yb-popup @close="close" :isShow="isShow" title="操作提示" width="350">
      ...
      <div slot="foot" style="line-heihgt:50px;height:50px;">
        <el-button type="defalut" @click="$router.push({name: 'mediPatrolRecordDailyList'})">返回
        </el-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        photo: [], //图片
        sign: [],  //电子签名
        selectMainbodyApiFn: postEntityPage, //选择主体的api方法
    ...
    watch: {
      'ruleForm.infoList': function () {
        this.ruleForm.soList.forEach(s => {
          s.disabled = false
          this.ruleForm.infoList.forEach(j => {
            if (j === s.id) {
              s.disabled = true
            }
          })
      ...
      'ruleForm.patrolSoId': function () {
        this.ruleForm.info.forEach(s => {
          s.disabled = false
          if (this.ruleForm.patrolSoId === s.id) {
            s.disabled = true
          }
        })
      }
    ...
    methods: {
      ...
      initInfo() {
        if (this.$store.getters.user.areaCode.length == 6) {
          //如果是区级则修改提示
          this.infoTitle = '陪同人员:'
          this.infoPlaceholder = '请选择陪同人员'
          //修改提示
          this.rules.infoList = []
          this.rules.infoList.push({ required: true, message: '请选择陪同人员', trigger: 'change' })
          this.rules.infoList.push({ validator: this.blurCount, trigger: 'change' })
        }
      },
      isNull(str) {
        if (str) {
        ...
        this.$refs['ruleForm'].validate((valid) => {
          if (valid) {
            this.$confirm('是否要提交巡查记录?', '提示', {
              ...
              let { entityId, patrolTime, score, result, treat, treatRemark,
                rectifyTime, isCooperate, patrolSoId, infoList,messName, patrolPlanType,
                entityName,entityType,planName,tableId,tableName,superviseName} = this.ruleForm;
              score = score + "";
              obj.record = {
                entityId,
                patrolTime,
              ...
              obj.record.license=this.ruleForm.backupNo;
      ...
      slmainbody(obj) {
        ...
        //获取基础信息
        getBasicInfo({ entityId: obj.entityId, type: obj.entityType }).then(res => {
          const data = res.data;
      ...
      getContext(obj) {
        //获取巡查项
        getContext({ tableId: obj.tableId })
          .then(res => {
            if (!res.status) {
      ...
      blurCount(rule, value, callback) {
        ...
        if (this.ruleForm.backupNo) {
          cnt1++;
        }
        if (this.ruleForm.patrolSoId != [] && this.ruleForm.patrolSoId.length>0) {
          cnt1++;
        }
