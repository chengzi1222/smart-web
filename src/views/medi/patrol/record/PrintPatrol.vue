  ...
  <div v-show="printShow" class="user-box" id="content">
    <div class="printContent">
      ...
      <div id="printBody">
        ...
        <p><span style="width:350px; display: inline-block;"> 法人：<u> {{basicInfoData.legalPerson}}</u></span>联系方式：
          <u> {{basicInfoData.mobile}}</u></p>
          <!-- 药品优化，针对限期整改2019-6.12 。。。start-->
        <p v-if="isShow&&patrolType=='整改复查'">
          <span style="width:350px; display: inline-block;"> 监管人员:<u> {{basicInfoData.patrolSoIdStr}}</u></span>
          <span style="width: 350px;display: inline-block;"> 信息员：<u> {{basicInfoData.messName}} </u></span>
        </p>
        <p v-if="!isShow&&patrolType=='整改复查'">
          <span style="width:350px; display: inline-block;"> 检查人员:<u> {{basicInfoData.patrolSoIdStr}}{{this.basicInfoData.areaCode.length}}</u></span>
        </p>
        <!-- 药品优化，针对限期整改2019-6.12。。。end -->

        <p v-if="patrolType!='整改复查'"><span style="width:350px; display: inline-block;"> 检查人：<u> {{basicInfoData.patrolSoIdStr}}</u></span>
          <span style="width: 450px;display: inline-block;" v-if='this.basicInfoData.areaCode'> {{ this.basicInfoData.areaCode.length == 6 ? '陪同人员':'信息员' }}：<u> {{basicInfoData.messName}} </u></span>
        <p>检查类别： <u> {{patrolType}} </u></p>
        <p>检查时间： <u> {{basicInfoData.patrolTime}}</u></p>
        <hr>
        <p class="text_indent">我们是<u> {{basicInfoData.deptName}}</u>的执法人员 <u>{{basicInfoData.patrolSoIdStr}}、{{basicInfoData.messName}}</u>，执法证件名称、编号是：
        ...
        <hr>
        <p class="text_indent" v-if="resultData.treat === 'TIME_LIMIT_RECTIFICATION'"> 针对上述问题，巡查结果为：<u>不符合</u>；处理方式为：<u>限期整改</u>。责令经营者务于
