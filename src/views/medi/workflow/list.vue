<template>
  <div class="user-box">
    ...
    <div class="ipt-btn-box" style="justify-content: end;">
      ...
      <enum-select class="selectHeight" v-model="condition.type" placeholder="业务类型" enum="com.ybveg.govx.enums.sub.SubWorkTypeEnum" @change="tabSearch"></enum-select>

    </div>
    <div class="tabOffTop50">
      <el-table :data="tableData" @sort-change="sortChange" class="table-div" v-show="condition.state != 'ALREADY_REJECT'">
        ...
        <el-table-column show-overflow-tooltip prop="gridName" min-width="140" label="分管网格"></el-table-column>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <a href="javascript:;" v-if="scope.row.state == 'PEND_REVIEW'" @click="review(scope.row,true)" class="operate-a" v-auth="'medi.biz.mb|func.review,medi.biz.eb|func.review'">审核</a>
  ...
  export default {
    ...
    methods: {
      ...
      searchPage() {
        ...
        getTotal({"type_ext":this.condition.type_ext}).then((result) => {
    ...
    data() {
      return {
        ...
        condition: {
          ...
          search: "",   //搜索
          sort: "DESC",
          type_ext:this.$route.params.type.toUpperCase()
        }
      }
    }
  }
