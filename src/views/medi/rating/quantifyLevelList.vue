  ...
  <div class="user-box">
    ...
    <div class="tabOffTop170">
      <el-table :data="tableData" class="table-div" @sort-change="handleSortChange" v-loading="loading">
        ...
        <el-table-column show-overflow-tooltip prop="quanLevel" min-width="120" label="量化等级（本年）"></el-table-column>
        ...
        <el-table-column @cell-mouse-enter="handleRegulationRecord" min-width="120" label="监管记录">
          <template slot-scope="scope">
            <span @mouseenter="takeMove(scope.row)" @mouseleave="takeLeve" style="overflow: hidden;white-space: nowrap;width: 109px;text-overflow: ellipsis;">
              当前量化等级：{{scope.row.quanLevel}}；当前风险等级：{{scope.row.riskRank}}； 动态评级：
              <span v-if="scope.row.dynaAssess && scope.row.dynaAssess.length > 0">
                <span v-for="(item,index) in scope.row.dynaAssess" :key="index">{{item.name}}：{{item.count}} </span>
              </span>
              <span v-else>暂无</span>
              巡查：共0次<span v-for="(item,index) in scope.row.patrolRecord" :key="index">
    ...
    <popupBody v-if="evaluate">
      ...
      <el-select class="w130" style="margin-right: 10px;" v-model="grade" placeholder="请选择">
        <el-option v-for="item in quantifyLevelList" :key="item.id" :label="item.name" :value="item.id" :disabled="item.id === 'NOASSESS'">
        </el-option>
      </el-select>
...
<script>
  ...
  export default {
    async mounted() {
      this.loading = true;
      var quantifyLevelDictCode = "MEDI_QUANTIFY";
